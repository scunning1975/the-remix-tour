...
\begin{center}
% TODO clip from Bacon (2021) JoE Figure 4 (event-study panel for SW)
% and save to figures/bacon_sw_eventstudy.png.
\includegraphics[width=0.7\textwidth,height=0.6\textheight,keepaspectratio]{figures/bacon_sw_eventstudy.png}
\end{center}
\vspace{-0.2em}
\begin{center}\small\color{warmgray}\itshape
SW's event study of female suicide rates relative to a state's no-fault-divorce reform year. \;Pre-period coefficients hover near zero; post-period coefficients turn negative and grow. \;Average post-treatment event-study coefficient: \highlight{$-4.92$ suicides per million women}.
\end{center}
\end{frame}

%% ---------- Slide 5: Bacon revisits ----------
\begin{frame}{Goodman-Bacon (2021) re-examined the SW estimate}
\vspace{0.5em}
Bacon (2021) ran the same TWFE regression on the same data --- the static specification, not the event study.

\vspace{0.6em}
\begin{center}\large
$\widehat{\beta}^{\,\text{TWFE}}_{\text{SW}} \;=\; -3.08 \quad\;\;(\text{156 components})$
\end{center}

\vspace{0.7em}
\begin{itemize}\small
  \item The late-treated-vs-already-treated 2$\times$2s average \highlight{$+3.51$} --- the \textit{wrong sign}, pulled there by treatment-effect heterogeneity over time.
  \item Drop those terms: the rest averages $-5.44$, very close to the event-study average of $-4.92$.
  \item The single TWFE coefficient of $-3.08$ masks a $+3.51$ vs.\ $-5.44$ split.
\end{itemize}

\vspace{0.4em}
\textbf{The puzzle this opens.} \;Why is TWFE's static-spec headline so much smaller in magnitude than the event-study average from the same data? \;\highlight{That's the question this morning.}
