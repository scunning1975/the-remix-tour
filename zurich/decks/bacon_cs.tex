...
\vspace{-0.4em}
\begin{center}\small\color{warmgray}\itshape
$\widehat{\beta}^{\text{TWFE}}$ converges to its estimand at $-3.08$ (LLN/CLT). \;\textit{Parallel trends does its job} --- every 2$\times$2 is a real ATT. \;\highlight{Heterogeneous treatment effects are the killer:} the forbidden 2$\times$2 picks up not just $\text{ATT}_\ell$ but also $-\Delta\text{ATT}_k$ from the already-treated control. \;Sign unknown without more structure.
\end{center}
\end{frame}
