...
\begin{frame}{Stevenson \& Wolfers (2006), revisited}
\vspace{0.4em}
...
\vspace{0.4em}
Plug these into Bacon's weighted sum. \;The TWFE probability limit is:
\begin{center}
\highlight{$\text{plim}\, \widehat{\delta}^{\text{TWFE}} \;=\; \text{VWATT} \;+\; \text{VWPT} \;-\; \Delta\text{ATT}$}
\end{center}
where VW means ``variance-weighted'' using the Bacon weights.
\end{frame}
...
%% ---------- L3-F1: The DGP ----------
\begin{frame}[fragile]{The DGP in 12 lines (baker.do)}
\begin{lstlisting}[language=Stata,basicstyle=\ttfamily\scriptsize]
* 40 states, 25 firms per state, 30 years (1980-2009)
* 4 cohorts treated at 1986, 1992, 1998, 2004
set seed 20200403
* ... set up the panel (state, firms, year) ...
...
gen y          = y0 + te_dynamic
\end{lstlisting}
\meta{One step per line, named variables, no loops. \;\texttt{simulations/staggered/baker.do}.}
\end{frame}
...
\vspace{0.5em}
For each $\text{ATT}(g,t)$, three options for the building-block estimator:

\vspace{0.5em}
\begin{enumerate}\small
  \item \textbf{Outcome Regression (OR).} \;Heckman-Ichimura-Todd: model $\Delta Y(0)$ on $X$ using controls, impute the treated counterfactual.
  \vspace{0.2em}
  \item \textbf{Inverse Propensity Weighting (IPW).} \;Abadie (2005): weight controls by $p(X)/(1-p(X))$.
  \vspace{0.2em}
  \item \textbf{Doubly Robust (DR).} \;Sant'Anna-Zhao (2020). \;Consistent if \textit{either} the OR model or the pscore model is correct.
\end{enumerate}

\vspace{0.6em}
\begin{block}{The practitioner's-guide recommendation}
\centering
``Favor doubly robust over RA or IPW alone.''
\end{block}

\vspace{0.5em}
\begin{center}
\small\color{warmgray}\itshape
The DR-DiD we proved yesterday is exactly the engine sitting inside each cell of CS.
\end{center}
\end{frame}
...
\vspace{0.5em}
\begin{itemize}
  \item Sensitivity analysis: how robust is CS to PT failing? \;(Rambachan-Roth.)
  \item Honest event-studies and how to read pre-trends without fooling yourself.
