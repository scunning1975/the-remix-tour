...
\begin{frame}{SA's proposition: $\mu_g$ is contaminated by other leads and lags}
\vspace{0.4em}
Even under parallel trends and no anticipation, the TWFE event-study coefficient on bin $g$ is:
\vspace{0.4em}
\begin{align*}
\mu_g \;=\; & \underbrace{\textstyle\sum_{l \in g} \sum_e w^g_{e,l}\, \text{CATT}_{e,l}}_{\text{the target (own bin)}} \\[0.2em]
            & + \underbrace{\textstyle\sum_{g' \neq g} \sum_{l' \in g'} \sum_e w^g_{e,l'}\, \text{CATT}_{e,l'}}_{\text{contamination from \textit{other} included bins}} \\[0.2em]
            & + \underbrace{\textstyle\sum_{l' \in g^{\text{excl}}} \sum_e w^g_{e,l'}\, \text{CATT}_{e,l'}}_{\text{contamination from \textit{excluded} bins}}
\end{align*}
\vspace{0.2em}
\highlight{Other-bin weights sum to zero --- but only cancel under treatment-effect homogeneity.} \;With heterogeneous CATTs, the contamination doesn't cancel.
\end{frame}
...
\begin{itemize}\small
  ...
  \item In TWFE it isn't --- it picks up CATTs from \textit{post-treatment} bins through the cross-bin weights.
  \item So a ``failed placebo'' may just be the cross-bin contamination.
  \item And a ``passing placebo'' may just be cancellation, not evidence of PT.
\end{itemize}
\vspace{0.5em}
...
\begin{frame}{The dCdH decomposition: weights on unit-level effects}
\vspace{0.4em}
Let $\Delta^g_{i,t} = Y^1_{i,t} - Y^\infty_{i,t}$ be the \textit{unit-level} treatment effect for unit $i$ in period $t$. Under PT, no anticipation, and SUTVA:
\vspace{0.4em}
\begin{block}{Theorem (dCdH 2020)}
\centering
$\delta^{\text{TWFE}} \;=\; \mathbb{E}\bigg[ \displaystyle\sum_{i,t:\, D_{i,t}=1} \tfrac{1}{N^T}\, w_{i,t}\, \Delta^g_{i,t} \bigg], \qquad \displaystyle\sum_{i,t:\, D_{i,t}=1} \tfrac{w_{i,t}}{N^T} = 1, \quad w_{i,t} \text{ \textbf{can be negative}}.$
\end{block}
\vspace{0.5em}
\begin{itemize}\small
  \item Weights come from the OLS residual of $D_{i,t}$ on unit and time FE.
  \item Bacon's weights are on \textit{2$\times$2s} (and always positive). \;dCdH's weights are on \textit{individual treatment effects} (and may be negative).
\end{itemize}
\end{frame}
...
\vspace{0.5em}
\textbf{Thought experiment.} \;Suppose every unit gains from the treatment ($\Delta^g_{i,t} > 0$ for all $i,t$).
\vspace{0.4em}
\begin{itemize}\small
  \item If all $w_{i,t} > 0$: $\widehat{\delta}^{\text{TWFE}}$ is a positive weighted average of positive effects. \;Sign matches.
  \item If some $w_{i,t} < 0$: nothing prevents $\widehat{\delta}^{\text{TWFE}}$ from being \highlight{negative} --- even though every unit gained.
\end{itemize}
\vspace{0.6em}
\textbf{This is what we saw in baker.do.} \;Truth $+82$, TWFE $-6.69$. Some unit-level effects got large negative weights; they swamped the positive-weight contributions.
\vspace{0.5em}
...
\vspace{0.5em}
\textbf{The diagnostic dCdH recommends running, before you publish anything:}
\vspace{0.4em}
\begin{enumerate}\small
  \item Fraction of unit-period treatment effects receiving \textit{negative} weight in your TWFE.
  \item The minimum amount of heterogeneity in $\Delta^g_{i,t}$ that would flip the sign of $\widehat{\delta}^{\text{TWFE}}$.
\end{enumerate}
\vspace{0.5em}
\textbf{Rules of thumb:}
\begin{itemize}\small
  \item Large never-treated group $\Rightarrow$ fewer negative weights, smaller magnitudes.
  \item Treatment dates packed close together $\Rightarrow$ also fewer.
  \item Long panel with a few early-treated cohorts $\Rightarrow$ the worst case.
\end{itemize}
\vspace{0.5em}
\meta{Software: \texttt{twowayfeweights} (Stata, SSC; R via the \texttt{TwoWayFEWeights} package) reports both diagnostics on your specific dataset.}
\end{frame}
...
\centering
The economic question is: \;\textit{what parameter do you want?} \\[0.3em]
OLS doesn't ask. \;OLS picks weights from the residual variance of $D$.
\end{block}
\vspace{0.7em}
\begin{itemize}
  \item Bacon: TWFE = weighted average of 2$\times$2s. \;Weights positive but variance-driven.
  \item SA: event-study $\mu_g$ = weighted average of CATT$_{e,l}$ \textit{across} bins. \;Cross-bin contamination.
  \item dCdH: TWFE = weighted average of unit-level $\Delta^g_{i,t}$. \;\highlight{Weights can be negative.}
\end{itemize}
\vspace{0.6em}
\textbf{All three say the same thing:} \;If you let OLS pick the weights, you've let it pick your economic question for you. \;Forward-engineer instead.
\end{frame}

%% =====================================================================
%%                                LESSON 8
%% =====================================================================
\remixsection{8.\ Synthesis \\[0.3em] {\normalsize\normalfont What's the same, what's different, what to do tomorrow morning}}
...
\begin{itemize}\small
  ...
  \item \textbf{If you must report it, also report CS, SA, or dCdH --- and the dCdH negative-weight diagnostic. Explain the differences.}
\end{itemize}
...
\begin{enumerate}\small
  ...
  \item \textbf{Re-estimate with CS or SA.} \;Compare the new headline to TWFE.
...
\vspace{0.5em}
\begin{itemize}
  \item \textbf{Honest event-studies.} \;Roth (2022) on the perils of conditioning on pretest passes. \;Rambachan \& Roth (2023) partial-identification bounds.
  \item \textbf{Imputation as a third path.} \;Borusyak-Jaravel-Spiess (2024) \;/\; Wooldridge (2021) extended TWFE \;/\; Gardner (2021): consistent estimators when no comparison group exists.
  ...
  \item \textbf{Goodman-Bacon, Sant'Anna, Whitten (2026)} --- a refined re-analysis framework for ``pre-trends parallel'' settings, with sharper conditions for when TWFE event-studies can be trusted.
\end{itemize}
