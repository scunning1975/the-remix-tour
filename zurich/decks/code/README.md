# `decks/code/` — Figure and table generators for the Zurich deck

This folder holds the **scripts that generate every figure and table** that appears in (or alongside) `basics.tex` and the other Zurich decks. The point is reproducibility: anyone with the source can recompile the deck and regenerate the visuals from scratch.
