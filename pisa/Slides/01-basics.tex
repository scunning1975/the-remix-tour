%% 01-basics.tex — Sant'Anna Workshop Pisa: Foundations of DiD
...
\begin{frame}{Day 1}
\vspace{0.3em}
{\large\highlight{9:00am -- 2:30pm} \quad\textbf{DiD intro and covariates}}
\begin{itemize}\setlength{\itemsep}{0.1em}\small
  \item Canonical 2$\times$2 difference-in-differences
  \item Conditional trends as functions of $X_i$
  \item Estimation under conditional parallel trends: regression adjustment, IPW, doubly-robust
  \item Sub-group heterogeneous treatment effects
  \item Coding lab: covariates
\end{itemize}
\vspace{0.4em}
{\large\highlight{2:30pm -- 5:00pm} \quad\textbf{Continuous DiD and coding lab}}
\begin{itemize}\setlength{\itemsep}{0.15em}\small
  \item DiD where the treatment is a non-negative ``dose'' (continuous DiD)
  \item Treatment effects under a dose: total effect and per-unit effect
  \item Interpretation difficulties: the dose-specific ATT
  \item Estimation of continuous DiD in 2$\times$2 settings
  \item Coding lab: continuous DiD
\end{itemize}
\end{frame}
...
\begin{frame}{Day 2}
\vspace{0.6em}
{\large\highlight{9:00am -- 2:00pm} \quad\textbf{Staggered treatment timing}}
\begin{itemize}\setlength{\itemsep}{0.15em}\small
  \item Staggered timing and the definition of effects: ATT$(g,t)$
  \item Aggregation of group-time treatment effects
  \item Callaway and Sant'Anna estimator for ATT$(g,t)$
  \item Imputation-based approaches
\end{itemize}
\vspace{0.5em}
{\large\highlight{2:00pm -- 5:00pm} \quad\textbf{Claude Code and DiD}}
\begin{itemize}\setlength{\itemsep}{0.15em}\small
  \item Claude Code introduction and Skills (but it seems down for me)
  \item Claude Code DiD Skills and example
\end{itemize}
\end{frame}
...
\vspace{1.4em}
\begin{center}
{\Large Four averages and three subtractions.}\\[1.8em]
...
\vspace{1.4em}
\begin{center}
{\Large Covariates}\\[1.8em]
{\large \highlight{TWFE}}\\[0.5em]
{\large \highlight{Outcome regression}}\\[0.5em]
{\large \highlight{Propensity scores}}\\[0.5em]
{\large \highlight{Doubly robust}}
\end{center}
\end{frame}

%% --- H.5 Today's focus ---
\begin{frame}{Today or tomorrow}
\vspace{1.4em}
\begin{center}
{\Large Continuous treatments}\\[1.8em]
{\large \highlight{TWFE}}\\[0.5em]
{\large \highlight{Target parameters}}\\[0.5em]
{\large \highlight{Identification}}\\[0.5em]
{\large \highlight{Estimation}}
\end{center}
\end{frame}
...
\vspace{1.4em}
\begin{center}
{\Large Staggered adoption}\\[1.8em]
{\large \highlight{Various estimators}}\\[0.5em]
{\large \highlight{Target parameters}}\\[0.5em]
{\large \highlight{Identification}}\\[0.5em]
{\large \highlight{Estimation}}
\end{center}
...
\vspace{1.4em}
\begin{center}
{\Large Synthetic control methods}\\[1.8em]
{\large \highlight{Abadie's original}}\\[0.5em]
{\large \highlight{Bias adjustment for poor fit}}\\[0.5em]
{\large \highlight{Synthetic diff-in-diff}}
\end{center}
\end{frame}
...
\vspace{1.4em}
\begin{center}
{\Large Claude Code}\\[1.8em]
{\large \highlight{To be determined due to some outage (for me)}}
\end{center}
\end{frame}
