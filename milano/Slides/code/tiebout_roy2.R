...
p <- ggplot(plot_data, aes(x = county_index, y = ate)) +
  geom_bar(stat = "identity", fill = "gray75", color = "gray60", linewidth = 0.2) +
  geom_hline(yintercept = overall_ate, linetype = "dashed", color = "gray30", linewidth = 0.7) +
  geom_hline(yintercept = county_level_ate, linetype = "dotted", color = "gray45", linewidth = 0.7) +
  annotate("text", x = total_counties * 0.85, y = overall_ate + 0.5,
           label = sprintf("Overall ATE = %.2f", overall_ate), color = "gray30", hjust = 1) +
  annotate("text", x = total_counties * 0.25, y = county_level_ate - 0.7,
           label = sprintf("County Average ATE = %.2f", county_level_ate), color = "gray45", hjust = 0) +
  labs(
