%% 04-staggered.tex — Italian Stata Conference Milano: Bacon decomposition + Callaway-Sant'Anna
%% Scott Cunningham, Italian Stata Conference, Milano, September 2026
%%
...
%%
%% Running example: baker.do simulated panel
...
%% ---------- Slide 2: TWFE intro ----------
\begin{frame}{The dominant estimator for staggered DiD}
\vspace{0.6em}
\begin{center}
\large
...
\begin{itemize}\setlength\itemsep{0.5em}
  \item Unit fixed effects $\alpha_i$ absorb time-invariant confounders
  \item Time fixed effects $\gamma_t$ absorb common shocks
  \item $\hat\delta$ was assumed to recover a ``reasonably weighted average'' ATT
\end{itemize}
\vspace{0.5em}
\begin{center}\color{warmgray}\small
For 30+ years: the workhorse for panel DiD with differential timing.
\end{center}
\end{frame}

%% ---------- Slide 3: The two papers ----------
\begin{frame}{Two papers changed everything}
\vspace{0.8em}
\begin{itemize}\setlength\itemsep{0.7em}
  \item \textbf{Goodman-Bacon (2021)} \textperiodcentered\ \textit{Journal of Econometrics}\\
        {\small\color{warmgray} TWFE $\hat\delta$ is a weighted average of \textit{all} underlying 2$\times$2 DiDs --- including comparisons that use already-treated units as controls.}

  \item \textbf{de Chaisemartin \& D'Haultf\oe{}uille (2020)} \textperiodcentered\ \textit{AER}\\
        {\small\color{warmgray} Under treatment effect heterogeneity, those weights can be negative --- wiping out the causal interpretation of $\hat\delta$.}
\end{itemize}
\vspace{0.7em}
\begin{center}
\highlight{Today: understand why, and learn what to do instead.}
\end{center}
\end{frame}
...
%% ---------- Bacon 2x2 image slides — all five ----------
\begin{frame}{The Bacon decomposition: all the 2$\times$2 comparisons}
\begin{center}
\includegraphics[width=0.82\textwidth,height=0.75\textheight,keepaspectratio]{figures/bacon_goodman_2.png}
\end{center}
\end{frame}

\begin{frame}{2$\times$2 \#1: early group $k$ vs.\ never-treated $U$}
\vspace{0.2em}
\begin{center}
\small
$\widehat{\delta}^{2\times 2}_{kU} \;=\; \bigl(\bar y_k^{\,\text{post}(k)} - \bar y_k^{\,\text{pre}(k)}\bigr) \;-\; \bigl(\bar y_U^{\,\text{post}(k)} - \bar y_U^{\,\text{pre}(k)}\bigr)$
\end{center}
\vspace{0.1em}
\begin{center}
\includegraphics[width=0.78\textwidth,height=0.65\textheight,keepaspectratio]{figures/bacon_goodman_3.png}
\end{center}
\end{frame}

\begin{frame}{2$\times$2 \#2: late group $\ell$ vs.\ never-treated $U$}
\vspace{0.2em}
\begin{center}
\small
$\widehat{\delta}^{2\times 2}_{\ell U} \;=\; \bigl(\bar y_\ell^{\,\text{post}(\ell)} - \bar y_\ell^{\,\text{pre}(\ell)}\bigr) \;-\; \bigl(\bar y_U^{\,\text{post}(\ell)} - \bar y_U^{\,\text{pre}(\ell)}\bigr)$
\end{center}
\vspace{0.1em}
\begin{center}
\includegraphics[width=0.78\textwidth,height=0.65\textheight,keepaspectratio]{figures/bacon_goodman_4.png}
\end{center}
\end{frame}

\begin{frame}{2$\times$2 \#3: $k$ vs.\ $\ell$ (not-yet-treated control --- clean)}
\vspace{0.2em}
\begin{center}
\small
$\widehat{\delta}^{2\times 2,\,k}_{k\ell} \;=\; \bigl(\bar y_k^{\textsc{Mid}} - \bar y_k^{\textsc{Pre}}\bigr) \;-\; \bigl(\bar y_\ell^{\textsc{Mid}} - \bar y_\ell^{\textsc{Pre}}\bigr)$
\end{center}
\vspace{0.1em}
\begin{center}
\includegraphics[width=0.78\textwidth,height=0.65\textheight,keepaspectratio]{figures/bacon_goodman_6.png}
\end{center}
\end{frame}

\begin{frame}{2$\times$2 \#4: $\ell$ vs.\ $k$ (already-treated control --- \highlight{forbidden})}
\vspace{0.2em}
\begin{center}
\small
$\widehat{\delta}^{2\times 2,\,\ell}_{\ell k} \;=\; \bigl(\bar y_\ell^{\textsc{Post}} - \bar y_\ell^{\textsc{Mid}}\bigr) \;-\; \bigl(\bar y_k^{\textsc{Post}} - \bar y_k^{\textsc{Mid}}\bigr)$
\end{center}
\vspace{0.1em}
\begin{center}
\includegraphics[width=0.76\textwidth,height=0.62\textheight,keepaspectratio]{figures/bacon_goodman_7.png}
\end{center}
\end{frame}
...
%% ---------- L5-F2b: When is CS used (ported verbatim from 03-ATTGT.tex) ----------
...
\begin{enumerate}\setlength\itemsep{0.35em}
  ...
  \item When short-run treatment effects are different than long-run effects
  \item When treatment effect dynamics differ if units are first treated in a recession relative to expansion years
\end{enumerate}
\vspace{0.6em}
\highlight{CS estimates the ATT by identifying smaller causal effects and aggregating them using non-negative weights.}
\end{frame}

%% ---------- L5-F2c: CS notation (ported verbatim from 03-ATTGT.tex) ----------
\begin{frame}{Notation}
\vspace{0.3em}
\begin{itemize}\small\setlength\itemsep{0.3em}
  \item $T$ periods going from $t = 1, \dots, T$
  \item Units are either treated ($D_t = 1$) or untreated ($D_t = 0$), but once treated cannot revert to the untreated state
  \item $G_g$ signifies a group and is binary. \;Equals one if individual units are treated at time period $t$
  \item $C$ is also binary and indicates a control-group unit, equalling one if \highlight{never treated} (can be relaxed though to \highlight{not yet treated})
  \begin{itemize}\footnotesize
    \item Recall the problem with TWFE using already-treated units as controls
  \end{itemize}
  \item Generalized propensity score enters into the estimator as a weight:
\end{itemize}
\vspace{-0.3em}
\[
\widehat{p(X)} \;=\; \Pr(G_g = 1 \mid X,\; G_g + C = 1)
\]
\end{frame}

%% ---------- L5-F2d: CS assumptions (ported verbatim from 03-ATTGT.tex) ----------
\begin{frame}{Assumptions}
\vspace{0.4em}
\small
\textbf{Assumption 1:} Sampling is iid (panel data, but repeated cross-sections are possible)
\vspace{0.5em}

\textbf{Assumption 2:} Conditional parallel trends (for either never-treated or not-yet-treated)
\vspace{-0.4em}
\[
\mathbb{E}[Y_t^0 - Y_{t-1}^0 \mid X, G_g = 1] \;=\; \mathbb{E}[Y_t^0 - Y_{t-1}^0 \mid X, C = 1]
\]
\vspace{-0.6em}

\textbf{Assumption 3:} Irreversible treatment
\vspace{0.4em}

\textbf{Assumption 4:} Common support (propensity score)
\vspace{0.4em}

\textbf{Assumption 5:} Limited treatment anticipation (i.e., treatment effects are zero pre-treatment)
\end{frame}

%% ---------- L5-F2e: CS IPW estimator (ported verbatim from 03-ATTGT.tex) ----------
\begin{frame}{CS Estimator (the IPW version)}
\vspace{0.6em}
\[
\text{ATT}(g,t) \;=\; \mathbb{E}\left[\left(\frac{G_g}{\mathbb{E}[G_g]}
 \;-\; \frac{\dfrac{\hat{p}(X)\,C}{1-\hat{p}(X)}}{\mathbb{E}\!\left[\dfrac{\hat{p}(X)\,C}{1-\hat{p}(X)}\right]}\right)\big(Y_t - Y_{g-1}\big)\right]
\]
\vspace{0.4em}
This is the inverse probability weighting estimator. \;Alternatively, there is an outcome regression approach and a doubly robust one. \;Sant'Anna recommends DR.
\vspace{0.5em}
\begin{center}
\highlight{CS uses the never-treated or the not-yet-treated as controls --- but never the already-treated.}
\end{center}
\end{frame}
...
%% =====================================================================
%%                                LESSON 7
%% ---------- L6-F7: SA vs CS, what actually differs ----------
\begin{frame}{SA vs.\ CS: what actually differs}
\scriptsize
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}lll@{}}
\toprule
 & \textbf{Callaway \& Sant'Anna} & \textbf{Sun \& Abraham} \\
\midrule
Building block & $\text{ATT}(g,t)$ cells & cohort $\times$ event-time $\delta_{e,l}$ \\
Comparison group & never-treated \textit{or} \highlight{not-yet-treated} & \highlight{never-treated} (or last-treated cohort) \\
Machinery & OR / IPW / DR, cell by cell & one saturated regression \\
Covariates & conditional PT, built in (DR) & would enter \highlight{additively} \\
Aggregation & any weighting you choose & cohort shares \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.2em}
\begin{itemize}\setlength\itemsep{0.15em}
  \item \textbf{Same principle.} \;Both forward-engineer: estimate the cells first, aggregate second. \;Neither ever uses the already-treated as a control.
  \item \textbf{The control group is a real difference.} \;CS can lean on the \textit{not-yet-treated}; SA's interaction-weighted estimator uses the \textit{never-treated} --- or, absent a never-treated group, the last-treated cohort.
  \item \textbf{Covariates are the sharper difference.} \;SA runs a regression, so $X$ would enter \textit{additively} --- and additive $X$ \highlight{imposes homogeneous treatment effects across $X$.} \;The SA paper does not cover covariates.
\end{itemize}
\end{frame}
...
%% =====================================================================
%%   SHORT-GAP vs LONG-DIFFERENCE  (ported from 03-ATTGT.tex)
%% =====================================================================
\remixsection{8.\ Short gaps vs.\ long differences \\[0.3em] {\normalsize\normalfont Two ways to compute the pre-treatment coefficients --- and the default is the wrong one}}
...
\begin{enumerate}\setlength\itemsep{0.5em}
  \item \textbf{Short gap.} \;Uses a ``rolling'' method in which a new baseline is used for each 2$\times$2 comparison.
  \vspace{-0.3em}
  \begin{align*}
  \widehat{\delta}_{t-3} \;=\;& \big(\mathbb{E}[Y \mid D{=}1, t{-}3] - \mathbb{E}[Y \mid D{=}1, t{-}2]\big) \\
  &- \big(\mathbb{E}[Y \mid D{=}0, t{-}3] - \mathbb{E}[Y \mid D{=}0, t{-}2]\big)
  \end{align*}
  \item \textbf{Long difference.} \;Uses a ``universal baseline'' with a fixed baseline at $t-1$.
  \vspace{-0.3em}
  \begin{align*}
  \widehat{\delta}_{t-3} \;=\;& \big(\mathbb{E}[Y \mid D{=}1, t{-}3] - \mathbb{E}[Y \mid D{=}1, t{-}1]\big) \\
  &- \big(\mathbb{E}[Y \mid D{=}0, t{-}3] - \mathbb{E}[Y \mid D{=}0, t{-}1]\big)
  \end{align*}
\end{enumerate}
\end{frame}
...
\vspace{1.2em}
TWFE and imputation methods (discussed next) \highlight{always use a fixed universal baseline.}
\vspace{1em}

To ensure comparability with CS or dCdH, be sure to use the universal-baseline syntax in Stata, R, or Python.
\end{frame}

\begin{frame}{Short gaps and long differences}
\begin{center}
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{figures/shortgap.png}
\end{center}
\end{frame}
...
\footnotesize
\begin{quote}
``A natural follow-up question is to what extent the event-study plots for these alternative methods can be modified to be more comparable to conventional event-study plots. For CS/dCdH, there is a straightforward solution, which is to use \emph{long-differences} for the pre-treatment coefficients as well as the post-treatment coefficients (i.e.\ always use the period before treatment as the baseline). This can be implemented, for example, in the \texttt{did} R and Stata packages using the options \texttt{base\_period = ``universal''} and \texttt{long2}, respectively. Using the default options in the \texttt{did\_multiplegt} Stata package also yields comparisons using \emph{long differences}, in contrast to the results for the R package shown above. Using these settings, the event-study estimates are numerically equivalent to the dynamic TWFE specification in the non-staggered setting considered here.''
\end{quote}
\vspace{0.2em}
\hfill --- Roth (2024)
\end{frame}

\begin{frame}[fragile]{Short vs.\ \texttt{long2} syntax in \texttt{csdid}}
\vspace{0.4em}
\footnotesize
...
\begin{frame}{Short gaps and long differences}
\begin{center}
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{figures/short_long.png}
\end{center}
\end{frame}
...
\vspace{0.5em}
\begin{itemize}\setlength\itemsep{0.5em}
  \item Unfortunately the \textcolor{red}{default} syntax for \texttt{csdid} and \texttt{did} in R uses \highlight{short gap}
  \item So you \emph{must} use \texttt{long2} or \texttt{base\_period = universal} to get the event-study coefficients that will correctly illustrate the pre-treatment coefficients
  \item In addition to Roth (2024), Brantly Callaway has a note on his website about it
\end{itemize}
\vspace{0.4em}
\begin{center}\scriptsize
\texttt{bcallaway11.github.io/posts/event-study-universal-v-varying-base-period}
\end{center}
\end{frame}
...
\begin{itemize}\small\setlength\itemsep{0.4em}
  ...
  \item But maybe the not-yet-treated is better --- why \textit{didn't} the never-treated adopt the treatment?
  \item \highlight{Don't get lost in the decisions and forget the importance of \emph{designing}} --- focus on the treatment assignment mechanism, checking imbalance
\end{itemize}
\end{frame}
...
{\small\color{warmgray} Scott Cunningham \textperiodcentered\ Italian Stata Conference \textperiodcentered\ Milano}
\end{frame}
