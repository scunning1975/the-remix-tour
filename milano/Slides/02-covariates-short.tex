%% 02-covariates-short.tex — Italian Stata Conference Milano: Covariates in DiD (shortened)
...
%% light gray for "this term vanishes" builds
\colorlet{faded}{warmgray!35}
\usepackage{amsmath}
...
%%
\begin{frame}{Recall: the 2$\times$2 --- the two arithmetics}
\vspace{0.8em}
{\small
...
\vspace{0.8em}
\begin{center}\color{warmgray}
Difference over time, then groups. \;Or groups, then time. \;Same number.
\end{center}
\end{frame}

%%
\begin{frame}{Recall: the 2$\times$2 --- the four regressions}
\vspace{0.6em}
{\small
\begin{enumerate}\setlength\itemsep{0.9em}
  \item[3.] $Y = \alpha_1 + \alpha_2\,\mathrm{Post} + \alpha_3\,D
         + \delta\,(\mathrm{Post}\times D) + \varepsilon$
        \hfill{\color{warmgray}\itshape saturated regression}

  \item[4.] $Y = \alpha_1 + \tau_t + \sigma_i
         + \delta\,(\mathrm{Post}\times D) + \varepsilon$
        \hfill{\color{warmgray}\itshape two-way fixed effects}

  \item[5.] $\Delta Y_i = \alpha_1 + \delta\,D_i + \varepsilon_i$
        \hfill{\color{warmgray}\itshape first-difference / horizontal regression}

  \item[6.] $\Delta\bar{Y}_G = \alpha_1 + \delta\,\mathrm{Post}_G + \varepsilon_G$
...
%%
\begin{frame}{Four ways to condition}
\vspace{1.2em}
\begin{enumerate}\setlength\itemsep{0.9em}\large
...
\vspace{1.2em}
\begin{center}
\large Just add in the covariates, then. \;Right?
\end{center}
\vspace{0.8em}
\begin{center}\color{warmgray}
Sure --- if you're willing to make two assumptions you haven't stated.
\end{center}
\end{frame}

%%
\begin{frame}{Two hidden assumptions --- and one fatal problem}
\vspace{0.8em}
...
%%
\begin{frame}{Subtract, and a $\Delta\beta \cdot X$ term survives}
\vspace{1em}
Subtract (post $-$ pre):
\[
\Delta Y_i(0)
...
\vspace{0.8em}
\begin{center}\color{warmgray}
The trend in $Y(0)$ now \textit{depends on} $X$. \;If the groups differ in $X$, their $Y(0)$ trends differ --- and that is exactly a parallel-trends violation.
\end{center}
\end{frame}
...
\vspace{1em}
\begin{center}\color{warmgray}
$\boldsymbol{\gamma}$ absorbs any shift in returns to $X$ between pre and post. \\[0.4em] (Spec B in the lab.)
\end{center}
\end{frame}

%%
\begin{frame}[fragile]{Fix for Bias \#1, in code}
\vspace{0.6em}
...
\vspace{1em}
\begin{center}\color{warmgray}
Interacting $D$ with every $X$ lets the treatment effect vary with $X$ --- \\ which is exactly what additive $X$ forbade.
\end{center}
\end{frame}

%%
\begin{frame}[fragile]{Fix for Bias \#2, in code}
\vspace{0.3em}
\begin{lstlisting}[language={[ANSI]C},basicstyle=\ttfamily\scriptsize]
...
%%
%%
\begin{frame}{But if you remember only two}
\vspace{1.2em}
\begin{enumerate}\setlength\itemsep{1.1em}
  \item[\textbf{1.}] \Large \textbf{Imbalance bias} \\[0.3em]
        {\normalsize\color{warmgray} The groups differ in an $X$ whose returns move over time, so their $Y(0)$ trends differ.}
  \item[\textbf{2.}] \Large \textbf{Heterogeneous treatment effect bias} \\[0.3em]
        {\normalsize\color{warmgray} The effect varies with $X$, and additive TWFE forbids that.}
\end{enumerate}
\vspace{1em}
\begin{center}\color{warmgray}\small
Bias \#3 is real but mechanical --- and it takes the same fix as \#1.
\end{center}
\end{frame}

%%
\begin{frame}{One thing we are not covering: covariates measured \textit{after} treatment}
\vspace{1.6em}
\begin{center}
\Large
Everything here assumes $X$ is measured at \highlight{baseline}.
\end{center}
\vspace{1.4em}
\begin{center}
Post-treatment covariates are their own problem, \\ and they have their own paper.
\end{center}
\vspace{1.2em}
\meta{Caetano and Callaway, forthcoming, \textit{Review of Economics and Statistics}.}
\end{frame}

%%
\begin{frame}{What that means in practice}
\vspace{1.4em}
\emphbox{Most researchers include additive $X$ and call it a day. \;That is the wrong spec.
\vspace{0.5em}

To fix all three you need \textit{both} $\mathrm{Post}\times X$ \textit{and} $D\times X$.
\vspace{0.5em}
...
%%
\begin{frame}{Same estimator, newer name}
\vspace{1.8em}
\begin{center}
\Large
You will increasingly see this called \\[0.4em] \highlight{regression adjustment} \;(RA).
\end{center}
\vspace{1.6em}
\begin{center}\color{warmgray}
Same idea. \;Newer name. \;Heckman, Ichimura and Todd got there in 1997.
\end{center}
\end{frame}
...
%%
\begin{frame}{Read that last row}
\vspace{1.8em}
\begin{center}
\Large
...
\vspace{1.6em}
\begin{center}\color{warmgray}
Two chances to be right is not the same as a guarantee. \\[0.4em] Get both models wrong and DR is wrong too.
\end{center}
\end{frame}
...
%% ====================================================================
%% CONCLUSION — choosing covariates, balance, overlap, separation
%% ====================================================================
\remixsection{In Conclusion \\[0.3em] {\normalsize\normalfont How to choose covariates, and how to check them}}

%%
\begin{frame}{This is not omitted variable bias}
\vspace{1.6em}
\begin{center}
\Large
The instinct is: \;\textit{``I forgot a variable, so I'll put it back in.''}
\end{center}
\vspace{1.4em}
\begin{center}\color{warmgray}
That instinct comes from cross-sectional regression. \\ It is the wrong instinct here.
\end{center}
\end{frame}

%%
\begin{frame}{A covariate here describes a \textit{population}}
\vspace{1.2em}
\begin{center}
\Large
$X$ is not a missing regressor.
\end{center}
\vspace{1em}
\begin{center}
\Large
$X$ marks \highlight{groups of people whose untreated outcomes} \\[0.3em] \highlight{were going to move differently anyway.}
\end{center}
\end{frame}

%%
\begin{frame}{And it is $Y(0)$, not $Y$}
\vspace{1.4em}
\begin{center}
\Large
The object is \;$\Delta \mathbb{E}[\,Y(0) \mid X\,]$
\end{center}
\vspace{1.2em}
\begin{center}\color{warmgray}
Not the level. \;Not the observed outcome. \;\\ The \textit{trend} in the \textit{untreated potential outcome}, within a group defined by $X$.
\end{center}
\end{frame}

%%
\begin{frame}{Our example, in those terms}
\vspace{1em}
\begin{center}
Men and women had different earnings trajectories \\ \highlight{whether or not anyone was treated.}
\end{center}
\vspace{1em}
\begin{center}\color{warmgray}
Sex did not ``cause'' the outcome and get left out of a regression. \\[0.4em]
Sex labelled two populations with different $\Delta Y(0)$. \\[0.4em]
The groups had different mixes of them. \;That is the whole problem.
\end{center}
\end{frame}

%%
\begin{frame}{So the common-sense method for choosing $X$}
\vspace{1em}
\begin{center}
Ask one question about each candidate covariate:
\end{center}
\vspace{1em}
\begin{center}
\Large\highlight{``Would this group's $Y(0)$ have trended differently?''}
\end{center}
\vspace{1.2em}
\begin{center}\color{warmgray}
If yes, it belongs. \;If it only predicts the \textit{level}, it does not.
\end{center}
\end{frame}

%%
\begin{frame}{Then two things to check}
\vspace{1.4em}
\begin{enumerate}\setlength\itemsep{1.2em}
  \item[\textbf{1.}] \Large \textbf{Balance} --- are the groups actually different in $X$?
  \item[\textbf{2.}] \Large \textbf{Overlap} --- can you find comparable controls at all?
\end{enumerate}
\end{frame}

%% ---------- 1. BALANCE ----------
\begin{frame}{1. \;Balance: the standardized difference in means}
\vspace{1.4em}
\[
\frac{\overline{X}_1 - \overline{X}_0}{\sqrt{\tfrac{1}{2}\big(V_1 + V_0\big)}}
\]
\vspace{1em}
\begin{center}\color{warmgray}
The gap between group means, scaled by the pooled standard deviation. \\ Unit-free, so it is comparable across variables.
\end{center}
\end{frame}
...
\begin{center}
\Large
$\left| \dfrac{\overline{X}_1 - \overline{X}_0}{\sqrt{\tfrac{1}{2}(V_1 + V_0)}} \right| \;>\; \highlight{0.25}$
\end{center}
\vspace{1.4em}
\begin{center}
If you have that, it is \textbf{imbalanced.}
\end{center}
\vspace{0.6em}
\meta{Imbens and Rubin (2015).}
\end{frame}

%%
\begin{frame}{And then including it is not optional}
\vspace{1.4em}
\begin{center}
\Large
Imbalanced \;$+$\; predicts $\Delta Y(0)$ \;$=$\; \highlight{you must condition on it}
\end{center}
\vspace{1.4em}
\begin{center}\color{warmgray}
Our sex example cleared both bars. \;That is exactly why the naive 2$\times$2 was wrong.
\end{center}
\end{frame}

%% ---------- 2. PROPENSITY SCORES ----------
\begin{frame}{2. \;Estimate the propensity score}
\vspace{1.4em}
\begin{center}
\Large
$p(X) \;=\; \Pr(D = 1 \mid X)$
...
\vspace{1.4em}
\begin{center}\color{warmgray}
One number per unit: \;how treated-looking is this observation?
\end{center}
\end{frame}

%%
\begin{frame}{Then plot it --- two histograms, one axis}
\vspace{1em}
\begin{center}
Plot the distribution of $p(X)$ \;\highlight{separately for treatment and control.}
\end{center}
\vspace{1.2em}
\begin{center}\color{warmgray}
This is the single most informative picture in the whole covariate discussion, \\ and almost nobody shows it.
\end{center}
\end{frame}

%%
\begin{frame}{Look for overlap}
\vspace{1.2em}
\begin{center}
\Large Do the two distributions \highlight{sit on top of each other?}
\end{center}
\vspace{1.2em}
\begin{center}\color{warmgray}
Where they overlap, you have comparable controls. \\[0.4em]
Where they don't, you are extrapolating.
\end{center}
\vspace{0.8em}
\begin{center}
If the tails misbehave: \;\textbf{consider trimming the control group.}
\end{center}
\end{frame}

%%
\begin{frame}{The IPW weight}
\vspace{1.4em}
\begin{center}
\Large
$\dfrac{p(X)}{1 - p(X)}$
...
\vspace{1.4em}
\begin{center}
\highlight{Applied \textit{only} to the comparison group.}
\end{center}
\end{frame}

%%
\begin{frame}{For the ATT, the treated units' propensity score is never used}
\vspace{1.6em}
\begin{center}
\Large
DiD estimates the \textbf{ATT}.
\end{center}
\vspace{1.2em}
\begin{center}\color{warmgray}
The treated group is already the population we want. \\[0.4em]
We do not reweight it. \;We reweight the controls \textit{to look like it}.
\end{center}
\end{frame}
...
\vspace{1.2em}
\[
\mathbb{E}\left[\left(\frac{D}{\mathbb{E}[D]}
\;-\; \frac{(1-D)\,\dfrac{p(X)}{1-p(X)}}{\mathbb{E}\!\left[(1-D)\dfrac{p(X)}{1-p(X)}\right]}\right)\Delta Y\right]
\]
\vspace{0.8em}
\begin{center}\color{warmgray}
Two terms. \;Look at what each one does.
\end{center}
\end{frame}

%%
\begin{frame}{When $D = 1$}
\vspace{0.9em}
\[
\mathbb{E}\left[\left(\frac{D}{\mathbb{E}[D]}
\;-\; {\color{faded}\frac{\overbrace{(1-D)}^{\mathclap{\;=\;0}}\,\dfrac{p(X)}{1-p(X)}}{\mathbb{E}\!\left[(1-D)\dfrac{p(X)}{1-p(X)}\right]}}\right)\Delta Y\right]
\]
\vspace{0.6em}
\begin{center}
The second term carries $(1-D)$. \;\highlight{It vanishes.}
\end{center}
\vspace{0.7em}
\begin{center}\color{warmgray}
Treated units enter with weight $1/\mathbb{E}[D]$. \\[0.4em]
Their propensity score is nowhere in the expression.
\end{center}
\end{frame}

%%
\begin{frame}{When $D = 0$}
\vspace{0.9em}
\[
\mathbb{E}\left[\left({\color{faded}\frac{\overbrace{D}^{\mathclap{\;=\;0}}}{\mathbb{E}[D]}}
\;-\; \frac{(1-D)\,\dfrac{p(X)}{1-p(X)}}{\mathbb{E}\!\left[(1-D)\dfrac{p(X)}{1-p(X)}\right]}\right)\Delta Y\right]
\]
\vspace{0.6em}
\begin{center}
The first term carries $D$. \;\highlight{It vanishes.}
\end{center}
\vspace{0.7em}
\begin{center}\color{warmgray}
Control units enter weighted by $p(X)/(1-p(X))$. \\[0.4em]
That is the IPW, and it applies to them alone.
\end{center}
\end{frame}

%%
\begin{frame}{Now watch what happens as $p(X) \rightarrow 1$}
\vspace{1.2em}
\begin{center}
\Large
$\dfrac{p(X)}{1-p(X)} \;\longrightarrow\; \infty$
\end{center}
\vspace{1.2em}
\begin{center}
A control unit that \textit{looks} treated gets \highlight{enormous} weight.
\end{center}
\vspace{1em}
\begin{center}\color{warmgray}
One observation can swamp the estimate. \;\textbf{This is why we trim.}
\end{center}
\end{frame}

%% ---------- 3. PERFECT SEPARATION ----------
\begin{frame}{3. \;A different problem: perfect separation}
\vspace{1.6em}
\begin{center}
\Large
Trimming fixes a \textit{tail}.
\end{center}
\vspace{1.2em}
\begin{center}
\Large
What if there is \highlight{no overlap at all?}
\end{center}
\end{frame}

%%
\begin{frame}{Where separation comes from}
\vspace{1.2em}
\begin{itemize}\setlength\itemsep{0.7em}
  \item \textbf{Overfitting.} \;Enough covariates and a flexible enough model will separate anything.
  \item \textbf{Genuinely selective $X$.} \;Selection on observables is so sharp that little randomness is left.
\end{itemize}
\vspace{1.2em}
\begin{center}\color{warmgray}
The first is a modelling mistake. \;The second is a fact about your design.
\end{center}
\end{frame}

%%
\begin{frame}{If it's real, you are near an RDD}
\vspace{1.4em}
\begin{center}
\Large
Regression discontinuity has \highlight{no overlap either} --- by construction.
\end{center}
\vspace{1.4em}
\begin{center}\color{warmgray}
Nobody just above the cutoff is comparable to anybody just below, \\ except in the limit. \;That is the whole design.
\end{center}
\end{frame}

%%
\begin{frame}{So IPW cannot help you}
\vspace{1.6em}
\begin{center}
\Large
Abadie (2005) needs \highlight{common support.}
\end{center}
\vspace{1.2em}
\begin{center}
Under separation you do not have it --- \\ and no amount of trimming creates it.
\end{center}
\end{frame}

%%
\begin{frame}{Reach for outcome regression instead}
\vspace{1.4em}
\begin{center}
\Large
...
\vspace{1.2em}
\begin{center}\color{warmgray}
Model the control trend, then \textit{impute} the missing counterfactual \\ for the treated units.
\end{center}
\end{frame}

%%
\begin{frame}{But understand what you just took on}
\vspace{1.4em}
\begin{center}
\Large
You are \highlight{imputing}.
\end{center}
\vspace{1.2em}
\begin{center}
The answer is now \textbf{highly dependent on functional form.}
\end{center}
\vspace{1em}
\begin{center}\color{warmgray}
IPW let the data pick comparable controls. \;OR makes you assert a shape.
\end{center}
\end{frame}

%%
\begin{frame}{Which means: be flexible, then be careful}
\vspace{1.2em}
\begin{itemize}\setlength\itemsep{0.8em}
  \item Use a \textbf{highly flexible specification} --- machine learning is reasonable here.
  \item Flexible models overfit. \;If yours does, consider \textbf{penalized regression.}
\end{itemize}
\vspace{1.2em}
\begin{center}\color{warmgray}
You are trading a support problem for a specification problem. \\ Do it with your eyes open.
\end{center}
\end{frame}

%%
\begin{frame}{The checklist, in one place}
\vspace{0.6em}
\begin{enumerate}\setlength\itemsep{0.5em}
  \item Choose $X$ that plausibly drives $\Delta Y(0)$ --- \textit{populations}, not omitted variables
  \item Check balance: standardized difference $> |0.25|$ means imbalanced
  \item Estimate $p(X)$; plot it by group; look for overlap
  \item Trim the control tail where $p(X) \rightarrow 1$
  \item If there is no overlap at all, stop --- IPW will not save you
  \item Use OR with a flexible spec, and penalize if it overfits
\end{enumerate}
\vspace{0.8em}
\meta{More on this in \textit{Causal Inference: The Remix}, the unconfoundedness chapter --- where I work through the Imbens-inspired checklists.}
\end{frame}
