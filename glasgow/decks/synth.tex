%% synth.tex — Glasgow Day 3 (afternoon): Synthetic Control + Synthetic DiD
%% Scott Cunningham, University of Glasgow, May 20, 2026
%%
...
\remixtitleframe
  ...
  {University of Glasgow \textperiodcentered\ May 20, 2026}
...
%% =====================================================================
%% INTUITION: the minimization, illustrated through nearest neighbor
%% =====================================================================

\begin{frame}{The intuition: start with a much simpler version of the problem}
\vspace{0.5cm}
...
\textbf{Suppose} you had a single treated unit, and you got to pick \textit{one} control from a pool. \;Which one would you pick?

\vspace{0.4cm}

\highlight{The one whose pre-period covariates are closest to the treated unit's.} \;Nearest neighbor. \;The same idea as synth --- just with a discrete choice instead of a weighted blend.

\vspace{0.4em}
\meta{Next: a toy example with 10 trainees and 20 candidate controls.}

\vspace{0.2em}
\meta{Worksheet: \href{https://docs.google.com/spreadsheets/d/1iro1Qzrr1eLDY_LJVzOYvnQZWmxY8JyTcDf6YcdhkwQ/edit?usp=sharing}{\color{ocean}\underline{Google Sheet}}}
\end{frame}

\begin{frame}{A toy example: 10 trainees, 20 candidate controls}
\vspace{0.5cm}

\textbf{Setup.}
\begin{itemize}\small\setlength\itemsep{1pt}
  \item 10 trainees (treated): \;known age and GPA.
  \item 20 candidates (controls): \;known age and GPA.
  \item Procedure: \;for each trainee, pick \textit{one} control. \;Compute the gap.
\end{itemize}

\vspace{0.5cm}

\textbf{Distance measure.} \;The Euclidean distance between the trainee sample and the picked controls:
\[
\text{discrepancy} \;=\; \sqrt{\,\sum_{i=1}^{10} \big[(\text{age}_i - \text{age}_{j(i)})^2 + (\text{gpa}_i - \text{gpa}_{j(i)})^2\big]\,}
\]

\vspace{0.3em}
\meta{The smaller this number, the better the match.}
\end{frame}

\begin{frame}{Pick one set of controls at random: discrepancy $=$ 45.76}
\vspace{0.6cm}
\begin{center}
\begin{tikzpicture}[every node/.style={font=\small}]
  \node[draw, rounded corners, fill=cream, minimum width=6cm, minimum height=2.4cm,
        text width=5.5cm, align=center, text=charcoal] at (0,0)
    {\textbf{Random pick \#1} \\[0.4em]
     \fontsize{34}{40}\selectfont\bfseries\color{accent}45.76 \\[0.3em]
     \footnotesize\color{warmgray}\itshape units of pre-period discrepancy};
\end{tikzpicture}
\end{center}

\vspace{0.7cm}

\textbf{What this number means.} \;The trainee sample and the chosen controls differ by 45.76 ``units'' in joint (age, GPA) space. \;\textit{Not} 45.76 years of age and \textit{not} 45.76 GPA points --- just 45.76 in the rescaled Euclidean metric.

\vspace{0.3em}
\meta{Can we do better with a different draw? \;Let's try.}
\end{frame}

\begin{frame}{Pick a different set: discrepancy $=$ 32.53}
\vspace{0.6cm}
\begin{center}
\begin{tikzpicture}[every node/.style={font=\small}]
  \node[draw, rounded corners, fill=cream, minimum width=11.5cm, minimum height=2.4cm,
        text width=11cm, align=center, text=charcoal] at (0,0)
    {\begin{tabular}{c@{\hspace{1.6cm}}c}
       \textbf{Random pick \#1} & \textbf{Random pick \#2} \\[0.3em]
       {\fontsize{30}{36}\selectfont\bfseries\color{warmgray} 45.76} &
       {\fontsize{30}{36}\selectfont\bfseries\color{accent} 32.53} \\[0.2em]
     \end{tabular}};
\end{tikzpicture}
\end{center}
...
The second random pick is closer. \;Two random picks, two different gaps. \;\highlight{The discrepancy depends on \textit{which controls you choose}.}

\vspace{0.3em}
\meta{If random sampling gives different gaps, there must be a \textit{best} pick. \;Can we find it?}
\end{frame}

\begin{frame}{The optimal nearest-neighbor pick: discrepancy $\approx$ 2.99}
\vspace{0.6cm}
\begin{center}
\begin{tikzpicture}[every node/.style={font=\small}]
  \node[draw, rounded corners, fill=cream, minimum width=13cm, minimum height=2.4cm,
        text width=12.5cm, align=center, text=charcoal] at (0,0)
    {\begin{tabular}{c@{\hspace{0.8cm}}c@{\hspace{0.8cm}}c}
       \textbf{Pick \#1} & \textbf{Pick \#2} & \textbf{Optimal} \\[0.3em]
       {\fontsize{24}{30}\selectfont\bfseries\color{warmgray} 45.76} &
       {\fontsize{24}{30}\selectfont\bfseries\color{warmgray} 32.53} &
       {\fontsize{32}{38}\selectfont\bfseries\color{accent} 2.99} \\[0.2em]
     \end{tabular}};
\end{tikzpicture}
\end{center}
...
\highlight{Searching over picks dramatically reduces the gap.} \;The optimum is a thousand-fold improvement over random.

\vspace{0.4em}
\meta{This \textit{is} the minimization. \;The next slide shows why it works.}
\end{frame}

\begin{frame}{Why optimization works: the gap is U-shaped in the weight}
\vspace{0.2cm}
\begin{center}
\begin{tikzpicture}[scale=0.9]
  % Axes
  \draw[->, thick, warmgray] (-4.3, 0) -- (4.5, 0) node[right, font=\small] {weight $w$};
  \draw[->, thick, warmgray] (0, -0.4) -- (0, 4.5) node[above, font=\small, text width=3cm, align=center] {discrepancy};

  % U-curve: y = 0.4 x^2 + 0.3
  \draw[thick, accent, smooth, samples=100, domain=-3.5:3.5]
       plot (\x, {0.4*\x*\x + 0.3});

  % Minimum point
  \filldraw[accent] (0, 0.3) circle (3pt);
  \node[below right=2pt, font=\small\bfseries, text=accent] at (0, 0.3) {optimal $w^*$};

  % Another point (worse)
  \filldraw[warmgray] (2.2, {0.4*2.2*2.2 + 0.3}) circle (3pt);
  \draw[dashed, warmgray!70] (2.2, 0) -- (2.2, {0.4*2.2*2.2 + 0.3});
  \node[right=3pt, font=\small, text=warmgray] at (2.2, {0.4*2.2*2.2 + 0.3 + 0.2}) {any other $w$};

  % Horizontal lines showing gap difference
  \draw[dashed, warmgray!70] (0, 0.3) -- (-3.8, 0.3);
  \draw[dashed, warmgray!70] (2.2, {0.4*2.2*2.2 + 0.3}) -- (-3.8, {0.4*2.2*2.2 + 0.3});
  \node[left, font=\scriptsize, text=accent] at (-3.8, 0.3) {min gap};
  \node[left, font=\scriptsize, text=warmgray] at (-3.8, {0.4*2.2*2.2 + 0.3}) {bigger gap};
\end{tikzpicture}
\end{center}

\vspace{0.1cm}
\begin{center}
\meta{The discrepancy is convex in the weight. \;One minimum. \;Any other choice is strictly worse.}
\end{center}
\end{frame}

\begin{frame}{From discrete picks to continuous weights}
\vspace{0.5cm}

\textbf{Nearest neighbor.} \;You picked one control per trainee. \;Discrete choice. \;The best you could do was 2.99.

\vspace{0.5cm}

\textbf{Synthetic control.} \;Don't pick one. \;Build a \textit{weighted blend} of all 20 candidates: \;$w_1$ on candidate 1, $w_2$ on candidate 2, $\ldots$, $w_{20}$ on candidate 20. \;Weights sum to 1; none negative.

\vspace{0.5cm}

\highlight{Same minimization problem.} \;The discrepancy is a function of the weight vector $W$, and we find the $W$ that brings it as close to zero as possible.

\vspace{0.4em}
\meta{Continuous weights give you a strictly richer set of possible ``matches'' than discrete picks --- and the optimum is often much closer to zero.}
\end{frame}

\begin{frame}{The convex hull: what the weights can --- and cannot --- do}
\vspace{0.2cm}
\begin{center}
\begin{tikzpicture}[scale=1.0]
  % Axes
  \draw[->, thick, warmgray] (-0.4, 0) -- (5.6, 0) node[right, font=\small] {covariate 1};
  \draw[->, thick, warmgray] (0, -0.4) -- (0, 4.5) node[above, font=\small] {covariate 2};

  % Donor points
  \filldraw[ocean] (1.0, 1.0) circle (3pt);
  \filldraw[ocean] (1.5, 3.2) circle (3pt);
  \filldraw[ocean] (3.0, 3.6) circle (3pt);
  \filldraw[ocean] (4.3, 2.5) circle (3pt);
  \filldraw[ocean] (4.0, 0.8) circle (3pt);
  \filldraw[ocean] (2.2, 0.4) circle (3pt);
  \filldraw[ocean] (2.4, 2.0) circle (3pt);   % interior donor
  \filldraw[ocean] (3.2, 1.6) circle (3pt);   % interior donor

  % Convex hull polygon (outline)
  \draw[ocean!70, thick, dashed]
        (1.0, 1.0) -- (2.2, 0.4) -- (4.0, 0.8) -- (4.3, 2.5)
        -- (3.0, 3.6) -- (1.5, 3.2) -- cycle;

  % Treated unit INSIDE the hull
  \filldraw[accent] (2.6, 2.2) circle (4pt);
  \node[right=3pt, font=\small\bfseries, text=accent] at (2.6, 2.2) {treated};

  % Labels
  \node[font=\scriptsize, text=ocean] at (5.0, 0.6) {donors};
  \node[font=\scriptsize, text=ocean!90, anchor=west] at (4.6, 3.4) {convex hull};
\end{tikzpicture}
\end{center}

\vspace{0.05cm}
\begin{center}
\meta{Weights $w_j \geq 0$ with $\sum w_j = 1$ produce a point \textit{inside} the polygon. \;If the treated unit is inside, the gap can in principle go to zero. \;If outside, the best match sits on the boundary.}
\end{center}
\end{frame}
...
%% ---------- L4-F0: Identification assumption (factor model) ----------
\begin{frame}{What SC actually assumes: a factor model}
\vspace{0.2cm}
...
\vspace{0.3em}
\begin{itemize}\small\setlength\itemsep{1pt}
  \item $\alpha_j$ --- unit fixed effect; \;$\tau_t$ --- common time trend.
  \item $X_j$ --- \textit{observed} covariates; \;$\beta_t$ time-varying loading.
  \item $\textcolor{remixgreendark}{Z_j}$ --- \textit{unobserved} heterogeneity; \;$\textcolor{remixgreendark}{\gamma_t}$ time-varying return.
  \item $\varepsilon_{jt}$ --- idiosyncratic noise.
\end{itemize}

\vspace{0.3em}
\highlight{TWFE DiD requires $\gamma_t = \gamma$ for all $t$ --- PT in every period, pre \textit{and} post. \;SC lets $\gamma_t$ move.}
\end{frame}

%% ---------- L4-F0b: Structure vs noise — why pre-period length matters ----------
\begin{frame}{Why a long pre-period: structure vs.\ noise}
\vspace{0.2cm}

Two terms in $Y_{jt}(0)$ vary across $j$ and $t$:
\begin{itemize}\small\setlength\itemsep{1pt}
  \item $\gamma_t Z_j$ --- \textbf{structure.} \;Persistent. \;The same $Z_j$ shows up year after year, just with a different return $\gamma_t$.
  \item $\varepsilon_{jt}$ --- \textbf{noise.} \;Independent across periods.
\end{itemize}

\vspace{0.4em}
SC fits weights on the \textit{pre-period} data. We want those weights to track $\gamma_t Z_j$, not the noise.

\vspace{0.4em}
\textbf{Short $T_0$:} \;the noise looks like signal. \;The fit overfits $\varepsilon$.

\vspace{0.2em}
\textbf{Long $T_0$:} \;noise averages out across periods. \;The fit picks up the persistent $Z_j$ loading.

\vspace{0.4em}
\highlight{Long pre-period $\Rightarrow$ match the structure, not the noise.}

\vspace{0.2em}
\meta{Abadie's bias bound (2021 JEL, Sec.\ 3.3) makes this precise: bias is decreasing in $T_0$, increasing in $\text{Var}(\varepsilon)$ and in the number of unobserved factors in $Z_j$.}
\end{frame}
...
\begin{enumerate}\small\setlength\itemsep{1pt}
  \item \textbf{Convex-hull condition.} \;The treated unit's pre-period covariates lie within the convex hull of the donors. \;If California is extreme on every dimension, no weighted combination of donors can match it.
  \vspace{0.2em}
  \item \textbf{Long pre-period.} \;Enough $T_0$ to separate $\gamma_t Z_j$ from $\varepsilon_{jt}$. \;Short pre-period $\Rightarrow$ overfit the noise.
  \vspace{0.2em}
  \item \textbf{Similar donor pool.} \;Donors should look like the treated unit on $X_j$ \textit{and} plausibly on $Z_j$. \;A bigger pool of dissimilar units inflates the bias bound, not shrinks it.
  \vspace{0.2em}
  \item \textbf{Good pre-period fit.} \;Pre-period RMSPE small relative to the post-period gap. \;If the synthetic doesn't track the unit \textit{before} treatment, nothing after is credible.
\end{enumerate}
\end{frame}
...
\vspace{0.3em}
\begin{itemize}\small\setlength\itemsep{1pt}
  \item \textbf{The treated unit is extreme.} \;California's pre-period values may sit at the edge of the donor distribution. \;The convex hull doesn't reach it. \;The synthetic can only approximate.
  \vspace{0.2em}
  \item \textbf{The pre-period is short.} \;Not enough periods to average out $\varepsilon$. \;The weights chase noise.
  \vspace{0.2em}
  \item \textbf{Many treated units.} \;The original method is built for $N_1 = 1$. \;If you have 10 treated states, do you run 10 separate fits? \;Combine them how?
\end{itemize}

\vspace{0.4em}
\highlight{Synthetic DiD relaxes (1) via a DiD-style level shift and gains efficiency from time weights.}

\vspace{0.2em}
\meta{For (3): penalized synth (Abadie--L'Hour 2021), microsynth (Robbins et al.\ 2017), augmented SC (Ben-Michael et al.\ 2021).}
\end{frame}
...
%% ---------- L5-F4: SDID nests SC and DiD as special cases ----------
\begin{frame}{SDID nests both SC and DiD}
\footnotesize
Set parts of SDID to defaults and you recover the other estimators exactly.

\vspace{0.5em}
\begin{itemize}\setlength\itemsep{4pt}
  \item \textbf{Additive systematic component} ($L_{it} = \alpha_i + \beta_t$): \;oracle $\widetilde{w}_i \to 1/N_{co}$ and $\widetilde{\lambda}_t \to 1/T_{pre}$. \;\highlight{SDID $\to$ DiD asymptotically.}
  \item \textbf{Set $w_0 = 0$ and $\zeta = 0$} (no intercept, no ridge) with $N_{tr}=1$: \;the Abadie--Diamond--Hainmueller weight problem exactly. \;\highlight{SDID $\to$ SC.}
\end{itemize}

\vspace{0.6em}
\highlight{Reading.} \;SC, DiD, and SDID are one estimator at three settings of $(w_0, \zeta, \lambda)$. \;Bad pre-trends? Time weights kick in. Sharp pre-trend match? Unit weights collapse to SC. Additive $L$? Weights go uniform and you have DiD.
\end{frame}

%% ---------- L5-F5: Prop 99 head-to-head ----------
\begin{frame}{Prop 99 revisited: what each estimator says}
\footnotesize
Same California data we just used in \S2 --- \highlight{five estimators, five answers} (paper Table 1, p.\,8).
\vspace{0.3em}
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{Estimate} (packs/year) & \textbf{SE} \\
\midrule
SDID & $-15.6$ & $(8.4)$ \\
...
\begin{itemize}\setlength\itemsep{2pt}
  \item SDID sits \textit{between} SC and DIFP --- SC's tight pre-period match \textit{and} a DiD-style intercept.
  \item \textbf{SDID's SE (8.4) is the smallest} despite being the more flexible estimator. \;Locality of weighting strips out predictable variation.
\end{itemize}
\end{frame}
...
\begin{itemize}\small
  ...
  \item No substantive regularization on time weights. \;\meta{Asymmetry is structural: errors are assumed independent across units but serially correlated within unit --- there's no within-time cross-unit correlation to ridge away.}
\end{itemize}
...
%% ---------- L8-F0: Why we drop the 2004 cohort ----------
\begin{frame}{Pre-flight check: SDID needs \textit{block} treatment}
\footnotesize
...
\vspace{0.5em}
\begin{block}{So what about staggered settings (like baker.do)?}
...
\vspace{0.4em}
\textbf{Practical implication for baker.do:} drop the always-treated 2004 cohort to create a clean block (1986/1992/1998 treated vs.\ never-treated). \;Same restriction we used for CS this morning. \;For richer multi-cohort designs, Ben-Michael, Feller \& Rothstein (2019) is the cousin paper.
\end{frame}
...
\vspace{0.5em}
\begin{itemize}
  \item \textbf{A \textit{block} of treated units.} \;SC was designed for $N_1 = 1$ (and runs unit-by-unit beyond that). \;SDID is designed for a block of treated states treated jointly at the same time.
  \vspace{0.3em}
...
\begin{itemize}
  ...
  \vspace{0.3em}
  \item \textbf{Standard errors --- depends on $N_{tr}$.} \;With $N_{tr}=1$ (e.g., Prop 99) only \textbf{placebo} is defined (jackknife collapses). \;Small $N_{tr}>1$: bootstrap is unstable; jackknife (conservative) and placebo are alternatives. \;Large $N_{tr}$: bootstrap is preferred.
  \vspace{0.3em}
...
%% ---------- L10-F0a: Opening — three lenses ----------
\begin{frame}{Which estimator should I use?}
\footnotesize
The choice between SC, DiD, and SDID follows from three things you can see \textit{before} running anything.

\vspace{0.6em}
\begin{enumerate}\setlength\itemsep{6pt}
  \item \textbf{Treatment timing.} \;One treated unit? A block? Staggered cohorts?
  \item \textbf{Pre-trends.} \;Parallel in raw data? Different in level? Different in slope?
  \item \textbf{Specific concerns.} \;Heterogeneous effects? Dynamic effects? Inference with $N_{tr}=1$?
\end{enumerate}

\vspace{0.8em}
\highlight{Three lenses --- three slides --- one decision.}

\vspace{0.4em}
\meta{The lenses interact. Read all three before choosing. The decision matrix at the end consolidates.}
\end{frame}

%% ---------- L10-F0b: Lens 1 — Treatment timing ----------
\begin{frame}{Lens 1: how many treated units, and when?}
\footnotesize
\textbf{Count the treated units and look at adoption dates.} \;The shape of $W_{it}$ drives the choice.
...
\toprule
\textbf{Setting} & \textbf{First-choice estimator} \\
\midrule
$N_{tr} = 1$, long pre-period & \textbf{SC} (or SDID with placebo SE) \\
$N_{tr} > 1$, single adoption date (block) & \textbf{SDID} \\
$N_{tr} > 1$, staggered adoption & \textbf{CS / dCdH} (or SDID block-by-block, App.\ VI) \\
$N_{tr}$ very large, parallel trends credible & \textbf{DiD} \\
\bottomrule
\end{tabular}
\end{center}
...
\vspace{0.5em}
\begin{itemize}\setlength\itemsep{2pt}
  \item SDID's headline theorem is for \textit{block} assignment. Staggered settings: the Bacon-decomp issues from this morning still apply.
  \item DiD is right when pre-trends genuinely line up --- don't over-engineer.
\end{itemize}
\end{frame}

%% ---------- L10-F0c: Lens 2 — Pre-trends ----------
\begin{frame}{Lens 2: what do the pre-trends look like?}
\footnotesize
\textbf{Plot the raw pre-trend lines.} \;What you see tells you which assumption is cheap and which is binding.

\vspace{0.4em}
\begin{center}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{p{0.46\textwidth} l}
\toprule
\textbf{Pre-period pattern} & \textbf{Use} \\
\midrule
Parallel slopes, similar levels & \textbf{DiD} (cheapest assumption) \\
Parallel slopes, different levels & \textbf{SDID} ($w_0$ absorbs the gap) \\
Non-parallel; donor reweighting matches exactly & \textbf{SC} (if convex hull holds) \\
Non-parallel; donor pool doesn't bracket treated & \textbf{SDID} or augmented SC \\
...
\vspace{0.5em}
\highlight{If pre-trends visibly diverge, DiD is the wrong tool.} \;The question is which weighting fix.
\end{frame}

%% ---------- L10-F0d: Lens 3 — Specific concerns ----------
\begin{frame}{Lens 3: what's your specific concern?}
\footnotesize
\textbf{Match the worry to the estimator that addresses it.}
...
\begin{tabular}{p{0.46\textwidth} l}
\toprule
\textbf{Specific concern} & \textbf{Use} \\
\midrule
Heterogeneous effects across cohorts & \textbf{CS} (reports ATT$(g,t)$) \\
Dynamic effects growing over time & CS event study; or \textbf{synthdid} ES aggregation \\
$N_{tr} = 1$, need inference & \textbf{SC} or \textbf{SDID with placebo} \\
Time-varying confounders $X_{it}$ & \textbf{DR-DiD} (yesterday); or residualize first \\
Audience needs transparent weights & \textbf{SC} (sparse donor pool visible) \\
Long panel, many states, suspect PT & \textbf{SDID} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.4em}
\meta{Lenses interact. $N_{tr}=1$ (Lens 1) plus parallel trends in raw data (Lens 2) means SC and DiD both work --- Lens 3 (transparency? inference? heterogeneity?) breaks the tie.}
\end{frame}
...
\vspace{0.4em}
\meta{Walkthrough: \texttt{glasgow/labs/Brazil/README.md} (extend the existing 9 steps with an SDID step 10). \;Data still at GitHub \texttt{mixtape/brazil.dta}.}
\end{frame}
...
{\small\color{warmgray} Scott Cunningham \textperiodcentered\ University of Glasgow \textperiodcentered\ May 20, 2026}
\end{frame}
