...
\begin{itemize}\small
  ...
  \item \textbf{Population.} \;Municipalities that \textit{actually got} a CAPS, in years \textit{after} they got it.
  \item \textbf{Weighting.} \;Group-time ATTs ($g,t$); we'll aggregate later.
\end{itemize}

\vspace{0.4em}
\highlight{Decide this \textit{first}.} \;Every downstream choice --- estimator, comparison group, aggregation --- is meant to identify \textit{this} parameter, not some other one.
\end{frame}
...
\begin{tabular}{lrrr}
\toprule
\textbf{Cohort} & \textbf{Municipalities} & \textbf{Share of treated} & \textbf{Max covariates (EPV $\leq$ 7)} \\
\midrule
...
\begin{center}
\meta{2003: smallest cohort $\to$ binds covariate count. \;2006: largest $\to$ dominates the simple-aggregation weight.}
\end{center}
\end{frame}
...
Two cohorts: \;\textbf{Group A} (5 post-period 2$\times$2's, 500 units), \;\textbf{Group B} (2 cells, 100 units).
...
2$\times$2        & $A_1$ & $A_2$ & $A_3$ & $A_4$ & $A_5$        & $B_1$ & $B_2$ \\
\midrule
ATT(g,t)          & 0.10 & 0.12 & 0.14 & 0.11 & 0.13           & 0.50 & 0.60 \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Two ways to aggregate:}
\begin{itemize}\small\setlength\itemsep{0pt}
  \item \textbf{Simple} (sample-weighted): \;$\frac{500(0.12) + 100(0.55)}{600} = \textbf{0.19}$
  \item \textbf{Group} (unweighted mean of group means): \;$\frac{0.12 + 0.55}{2} = \textbf{0.34}$
\end{itemize}

\vspace{0.15em}
\highlight{Same data, two answers.}
\end{frame}
...
\vspace{0.2em}
\meta{\texttt{mvec(0(0.25)2)} \;= sweep $M$ from 0 to 2 in steps of 0.25. \;\texttt{delta(rm)} \;= relative-magnitudes restriction (the standard choice).}
\end{frame}

\begin{frame}{Reading the HonestDID plot}
\vspace{0.4cm}

The output is a \textbf{forest of confidence intervals}, one for each $M$.

\vspace{0.4cm}
\begin{itemize}\setlength\itemsep{3pt}
  \item $M = 0$: \;the original CS estimate. \;Tight CI, point estimate.
  \item $M = 0.5$: \;allow post-period PT violation half as large as the worst pre-period one.
  \item $M = 1$: \;allow it as large as the worst pre-period one.
  \item $M = 2$: \;allow twice as large.
\end{itemize}

\vspace{0.4cm}

\textbf{The reading:} \;the value of $M$ at which the CI just touches zero is the ``breakdown point.'' \;If your conclusion survives $M \geq 1$, the evidence is robust to a PT violation as large as anything you actually see in the pre-period.

\vspace{0.4em}
\meta{This isn't a hypothesis test. \;It's a discipline: \;state what you'd need to believe for your conclusion to fail, and let the reader judge.}
\end{frame}
