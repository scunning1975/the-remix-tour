...
\begin{center}
\footnotesize
$\underbrace{\;\;\tau \text{ doesn't depend on } X\;\;}_{\textstyle\color{accent}\downarrow\;\;\textbf{Bias}_{\text{HTE}}}
\;\;\;
\underbrace{\;\;X \text{ has parallel trends across groups}\;\;}_{\textstyle\color{accent}\downarrow\;\;\textbf{Bias}_{\Delta X}}
\;\;\;
\underbrace{\;\;X \text{ varies within unit}\;\;}_{\textstyle\color{accent}\downarrow\;\;\textbf{Bias}_{\text{FE}}}$
\end{center}

\vspace{0.6em}
\begin{itemize}\setlength\itemsep{1pt}\footnotesize
  \item $\textbf{Bias}_{\textbf{HTE}}$. \;If $\tau(X)$ varies with $X$, TWFE fits one $\delta$. \;\textit{Y(1) problem.}
  \item $\textbf{Bias}_{\boldsymbol{\Delta X}}$. \;If $X$ is imbalanced and drives $Y(0)$'s trend, PTA fails. \;\textit{Y(0) problem.}
  \item $\textbf{Bias}_{\textbf{FE}}$. \;Time-invariant $X$ gets eaten by within-unit demeaning. \;Needs $\text{Post}\times X$.
\end{itemize}
\end{frame}
...
\textbf{What TWFE-additive-$X$ fits:} \;one scalar $\hat{\delta}$. \;A weighted average of $\tau(X)$ whose weights are determined by OLS mechanics, \textit{not} the treated $X$-distribution.
...
* Y(1) = Y(0) + tau(X). Treatment effect grows with age.
gen y1 = y0 + 200 + 50*age           // tau(age) = 200 + 50*age
gen tau = y1 - y0                    // varies by age

* True ATT (over treated X-distribution)
summarize tau if treat == 1
\end{lstlisting}

\vspace{0.4em}
\begin{itemize}\setlength\itemsep{2pt}
  \item Look at $\tau$: \;\;$\tau_i = 200 + 50\,\text{age}_i$. \;Not a constant.
  \item TWFE-additive-$X$ on the panel returns one $\hat{\delta}$. \;That number is a weighted average of $\tau$ that doesn't match $\mathbb{E}[\tau \mid D=1]$.
\end{itemize}
\end{frame}
...
\vspace{0.5em}
\highlight{In plain English.} \;You thought you controlled for race in LaLonde. \;The regression didn't see race. \;That's why additive-$X$ on LaLonde gives the same answer as no covariates at all.
\end{frame}
...
\vspace{0.4em}
\begin{itemize}\setlength\itemsep{2pt}
  \item $(\text{post}\times X)$ handles $\text{Bias}_{\Delta X}$: lets $X$'s effect shift at the policy break.
  \item $(\text{post}\times Z)$ handles $\text{Bias}_{\text{FE}}$: rescues time-invariant baseline characteristics from the FE.
\end{itemize}

\vspace{0.5em}
\textbf{What this spec \textit{cannot} fix:} \;$\text{Bias}_{\text{HTE}}$. \;If $\tau$ depends on $X$, no single $\hat\delta$ recovers the ATT.

\vspace{0.4em}
\highlight{The next move: estimators that handle $\text{Bias}_{\text{HTE}}$ by averaging $\tau(X)$ over the treated $X$-distribution explicitly.}
\end{frame}

%% ---------- L2-F2o2: Fully saturated TWFE = OR/IPW/DR semiparametrically ----------
\begin{frame}{Fully saturated TWFE \textit{is} what OR/IPW/DR do}
\footnotesize
\textbf{To handle $\text{Bias}_{\text{HTE}}$ inside TWFE, you'd saturate:}

\vspace{-0.2em}
\[
Y \;=\; \alpha_i + \gamma_t + (D{\times}\text{post})\,\big[\,\delta_0 + X'\delta_X\,\big] + X'\gamma + (\text{post}{\cdot} X)'\gamma_p + (D {\cdot}X)'\gamma_D + \cdots
\]

Every covariate gets $D{\times}X$, $\text{post}{\times}X$, and $D{\times}\text{post}{\times}X$ interactions. \;Then average the fitted $\tau(X)$ over the treated $X$-distribution to get the ATT.

\vspace{0.5em}
\textbf{This works.} \;It's also unwieldy --- 100 interaction terms for 25 covariates, and you have to compute the ATT yourself.

\vspace{0.5em}
\highlight{OR, IPW, and DR are doing exactly this --- semiparametrically.} \;Each models $\widehat{\mu}_0(X)$ or $\widehat{p}(X)$ once and uses it where the saturation would have gone.

\vspace{0.3em}
\meta{OR/IPW/DR aren't magic. They're efficient implementations of saturated TWFE.}
\end{frame}
