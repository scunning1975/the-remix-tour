...
%% ---------- L2-F0a: Lead with three biases (C-C decomp slide removed — redundant with the PO slides) ----------
\begin{frame}{TWFE-with-controls doesn't identify the ATT --- three biases}
\vspace{0.6em}
The workhorse: \;$Y_{it} = \alpha_i + \gamma_t + \delta\,(D_i {\times} \text{post}_t) + \beta\,X_{it} + \varepsilon_{it}$.
\vspace{0.6em}
\begin{itemize}\setlength\itemsep{4pt}
  \item Even under conditional PT, $\widehat{\delta}$ is \textit{not} the ATT in general.
  \item Three different biases can creep in. Each comes from a separate assumption you didn't realize you were making.
  \item Each gets its own slide next: \;\textbf{A} (heterogeneous $\tau$), \;\textbf{B} ($X$-trends differ), \;\textbf{C} (FE eats time-invariant $X$).
\end{itemize}

\vspace{0.4em}
\meta{All three are formalized in Caetano \& Callaway (2024); we'll cite back to them at the synthesis.}
\end{frame}
