%% covariates.tex — Glasgow Day 2 (afternoon): Covariates in DiD
...
\begin{enumerate}\small
  ...
  \item Compositional change (Sant'Anna \& Xu)
...
\begin{center}
\footnotesize
$\underbrace{\;\;\tau \text{ doesn't depend on } X\;\;}_{\textstyle\color{accent}\downarrow\;\;\textbf{Bias A}}
\;\;\;
\underbrace{\;\;X \text{ is balanced across groups}\;\;}_{\textstyle\color{accent}\downarrow\;\;\textbf{Bias B}}
\;\;\;
\underbrace{\;\;X \text{ varies within unit}\;\;}_{\textstyle\color{accent}\downarrow\;\;\textbf{Bias C}}$
\end{center}

\vspace{0.7em}
\begin{itemize}\footnotesize
  \item \textbf{Bias A.} \;If $\tau(X)$ is heterogeneous, TWFE fits one $\delta$ --- the wrong weighted average.
  \item \textbf{Bias B.} \;If $X$ is imbalanced across groups and matters for $Y(0)$, the gap shows up in DiD as fake treatment.
  \item \textbf{Bias C.} \;If $X$ is time-invariant, the FE transformation eats it. \;Time-invariant $Z$ needs $\text{Post}\times Z$.
\end{itemize}
\end{frame}

%% ---------- L2-F0z: One more setup assumption — X doesn't respond to treatment ----------
\begin{frame}{One more setup: $X$ doesn't respond to treatment}
\footnotesize
\textbf{When we condition on $X$, we treat $X$ as a fixed characteristic of unit $i$.}
...
\vspace{0.6em}
The value of $X$ is the same whether unit $i$ is treated or not. \;Equivalently: \;\textit{the treatment does not change the covariate.}

\vspace{0.5em}
\begin{itemize}\setlength\itemsep{3pt}
  \item If treatment changes $X$ (e.g., training raises motivation, which we measured as $X$), then $X$ is a \highlight{bad control}. \;Conditioning introduces post-treatment bias (Angrist--Pischke).
  \item Safe move: use the \textit{pre-period} value $X_{i,0}$. \;Treatment hasn't happened yet, so $X_{i,0}(1) = X_{i,0}(0)$ by construction.
\end{itemize}

\vspace{0.4em}
\meta{Implicit in everything that follows --- OR, IPW, DR all condition on $X$ and need this to hold.}
\end{frame}

%% ---------- L2-F0b1: Bias A in potential outcomes ----------
\begin{frame}{Bias A: heterogeneous $\tau$ in $X$}
\footnotesize
\textbf{In potential outcomes:}
\[
\tau(X) \;\equiv\; \mathbb{E}[Y(1) - Y(0) \mid X,\,D{=}1] \;\;\text{is a \textit{function} of } X.
\]
The true ATT averages $\tau(X)$ over the treated $X$-distribution:
\[
\text{ATT} \;=\; \mathbb{E}\big[\tau(X) \mid D{=}1\big].
\]

\textbf{What TWFE-additive-$X$ fits:} \;one scalar $\hat{\delta}$ --- a weighted average of $\tau(X)$ whose weights depend on OLS mechanics (the variance of $D$ given $X$), \textit{not} the treated $X$-distribution.

\vspace{0.5em}
\highlight{In plain English.} \;Treatment works better for some types of people than others (e.g., training helps younger workers more). \;TWFE fits one number for everyone --- a different weighted average than the one you wanted.
\end{frame}

%% ---------- L2-F0b2: Bias B in potential outcomes ----------
\begin{frame}{Bias B: $X$ moves differently over time across groups}
\footnotesize
\textbf{For PT-on-$Y$ to hold under the TWFE-additive-$X$ model:}
\[
...
\vspace{0.5em}
\highlight{In plain English.} \;The covariate trends differently for treated and control --- say, the policy nudged $X$ for one group but not the other --- and because $Y$ depends on $X$, that group difference shows up in the DiD as a fake treatment effect.
\end{frame}

%% ---------- L2-F0b3: Bias C in potential outcomes ----------
\begin{frame}{Bias C: the FE transformation eats time-invariant covariates}
\footnotesize
\textbf{Within-unit demeaning:} \;$\tilde{X}_{it} = X_{it} - \bar{X}_i$. \;If $X$ is time-invariant for unit $i$ (race, education at baseline, pre-program earnings), then $\tilde{X}_{it} = 0$ in every period.

\vspace{0.5em}
\[
\text{TWFE-additive-}X \;\;\text{ estimates as if those covariates were} \;\; 0.
\]
The same goes for any time-invariant $Z$. \;\;\textit{What you wrote as a control isn't what the regression conditioned on.}

\vspace{0.5em}
\textbf{The fix:} \;$Z$ has to enter as $(\text{Post}\times Z)$ --- which the FE cannot eat, because post varies within unit.

\vspace{0.5em}
\highlight{In plain English.} \;You thought you controlled for race in LaLonde. The regression didn't see race --- it demeaned it to zero. That's why additive-$X$ on LaLonde gives the same answer as no covariates at all.
\end{frame}
...
\begin{frame}[fragile]{Heterogeneous $\tau(X)$ in code: build $Y(0)$ and $Y(1)$ by hand}
\footnotesize
...
\begin{itemize}\setlength\itemsep{1pt}
  ...
  \item $\tau_i = 2500 + 100\,\text{age}_i + 1000\,\text{gpa}_i$ --- a 1-year-older worker gets \$100 more from training; a 1-GPA-point higher worker gets \$1{,}000 more.
...
\begin{itemize}\setlength\itemsep{4pt}
  ...
  \item \textbf{DGP 2 --- heterogeneous $\tau$ in $X$.} \;$\tau_i = 2 + 4(X_i - \tfrac{1}{2})$. \;Treatment effect grows with $X$.
  \item \textbf{DGP 3 --- time-invariant $Z$.} \;$Y(0)$ depends on a baseline $Z$ only in post. \;FE within-unit transformation \textit{eats} $Z$.
\end{itemize}
\vspace{0.5em}
\highlight{True $\tau = 2$ in every DGP.} \;Selection: treated units' $X$ shifts up by $0.3$ in post (the typical empirical pattern).
\end{frame}
...
\begin{frame}{Results: which spec fixes which bias? \;\;(true $\tau = 2$)}
...
\textbf{DGP 2} het $\tau$ in $X$    & \cellcolor{biasedred}$3.51$ & \cellcolor{biasedred}$2.91$ & \cellcolor{biasedred}$2.64$ & \cellcolor{biasedred}$2.64$ \\
...
\begin{center}
\footnotesize\color{warmgray}\itshape
\colorbox{remixgreenlight}{\strut\,green\,}: spec recovers $\tau \approx 2$. \quad \colorbox{biasedred}{\strut\,red\,}: spec is biased.
\end{center}
\end{frame}
...
\begin{itemize}\setlength\itemsep{4pt}
  ...
  \item \textbf{Heterogeneous $\tau$ in $X$} \;$\to$ \;Post${\times}X$ \textit{still misses it.} \;You need a saturated $D{\times}X$ interaction, or DR-DiD (Lesson 4), or matching.
...
\footnotesize
\textbf{To kill Bias B and Bias C inside TWFE, you need:}
\[
Y_{it} \;=\; \alpha_i + \gamma_t + \delta\,(D_i{\times}\text{post}_t) \;+\; X_{it}'\gamma_X \;+\; (\text{post}_t \cdot X_{it})'\gamma_{pX} \;+\; (\text{post}_t \cdot Z_i)'\gamma_{pZ} \;+\; \varepsilon_{it}
\]

\vspace{0.4em}
\begin{itemize}\setlength\itemsep{2pt}
  \item $(\text{post}\times X)$ handles Bias B: lets $X$'s effect shift at the policy break.
  \item $(\text{post}\times Z)$ handles Bias C: rescues time-invariant baseline characteristics from the FE.
\end{itemize}

\vspace{0.5em}
\textbf{What this spec \textit{cannot} fix:} \;Bias A. \;If $\tau$ depends on $X$, no single $\hat\delta$ recovers the ATT.

\vspace{0.4em}
\highlight{The next move: estimators that handle Bias A by averaging over the treated $X$-distribution explicitly.}
\end{frame}
...
%%                                LESSON 5
%% =====================================================================
\remixsection{5.\ Compositional Change}
...
%% =====================================================================
%%  CLOSING EXAMPLE — LaLonde: history, then the four specs, then synthesis
%% =====================================================================

%% ---------- LL-Hist-1: The empirical crisis of the 1970s ----------
\begin{frame}{The empirical crisis of the 1970s}
\vspace{0.5em}
Empirical economics in the 1970s did not have a strong reputation.
\vspace{0.5em}
\begin{itemize}\setlength\itemsep{3pt}
  \item Most empirical work was a postscript to a theory paper. \;Sargent on rational expectations is a representative case --- the regression was an afterthought.
  \item IV in the 1970s could be done without even \textit{reporting} the instrument. \;\meta{Orley Ashenfelter and Janet Currie, in conversation.}
  \item Outlandish estimates were routine and rarely interrogated.
\end{itemize}
\vspace{0.5em}
\highlight{There was a sense that econometric methods couldn't quite be trusted} --- and no shared standard for adjudicating that.
\end{frame}

%% ---------- LL-Hist-2: The Lewis smell test ----------
\begin{frame}{The Lewis smell test}
\vspace{0.5em}
\textbf{H. Gregg Lewis} (Chicago) studied union wage differentials. \;In his book reviewing the literature, he simply \textit{ignored} estimates he couldn't justify --- particularly when they came from IV or Heckman selection corrections.

\vspace{0.5em}
He preferred OLS with controls and a story you could defend.

\vspace{0.7em}
\textbf{Card called this the ``Lewis smell test.''}
...
%% ---------- LL-Hist-3: The other world — program evaluation ----------
\begin{frame}{The other world: program evaluation}
\vspace{0.5em}
\textbf{There was one corner of empirical work that took the question seriously.}

\vspace{0.5em}
\begin{itemize}\setlength\itemsep{3pt}
  \item Federal job-training programs in the 1970s --- evaluations commissioned by the Department of Labor.
  \item Princeton's \highlight{Industrial Relations Section} --- older than the Econ department --- ran the empirical agenda.
  \item Job training was a \textit{totem} in the Section. \;Heckman used it as a recurring metaphor (including the 1997 ReStud with Ichimura, Todd, and the later Smith papers).
\end{itemize}
...
%% ---------- LL-Hist-4: Orley (and Card) and their students ----------
\begin{frame}{Orley and Card's students}
\vspace{0.6em}
...
\vspace{0.6em}
\begin{itemize}\setlength\itemsep{3pt}
  \item \textbf{Ron Oaxaca} \;$\to$\; the wage decomposition (1973). \;\meta{Orley.}
  \item \textbf{Bob LaLonde} \;$\to$\; the job-training paper (1986). \;\meta{Orley and Card.}
  \item \textbf{Josh Angrist} \;$\to$\; Vietnam draft-lottery IV (1990). \;\meta{Orley and Card.}
\end{itemize}

\vspace{0.6em}
\highlight{Angrist once said: ``I wish I had written LaLonde.''}
...
%% ---------- LL-Hist-5: NSW + Orley's suggestion ----------
\begin{frame}{The NSW experiment, and Orley's suggestion}
\vspace{0.5em}
\textbf{The National Supported Work Demonstration (NSW).} \;Late-1970s federal job-training program run by MDRC. \;Hard-to-employ adults. \;\highlight{Randomly assigned} treatment and control.

\vspace{0.6em}
\textbf{Orley to LaLonde:}
\vspace{-0.2em}
\begin{center}
\textit{``Estimate the ATT from the experiment --- that's the truth. \\Then drop the experimental control. Replace it with survey data. \\Re-run the contemporary econometric methods. See what you get.''}
\end{center}

\vspace{0.5em}
\begin{center}
\small\color{warmgray}\itshape
A placebo-style design \textit{before} placebo designs were a thing. \;A direct honesty test.
\end{center}
\end{frame}

%% ---------- LL-Hist-6: What LaLonde found ----------
\begin{frame}{What LaLonde found (1986)}
\vspace{0.6em}
\textbf{The truth from the RCT:} \;ATT $\approx +\$800$.

\vspace{0.6em}
\textbf{What observational methods gave:} \;all over the map. \;Wrong magnitudes. \;Often \textit{wrong signs}.

\vspace{0.8em}
\begin{center}
\highlight{The conclusion --- and it cratered the credibility of observational program evaluation:}
\end{center}

\vspace{0.4em}
\begin{center}
\textit{The standard econometric methods of the day could not be trusted to recover known causal effects.}
\end{center}
\end{frame}

%% ---------- LL-Hist-7: Dehejia-Wahba revisit ----------
\begin{frame}{Dehejia \& Wahba (1999, 2002): the rescue}
\vspace{0.4em}
Mid-1990s. \;Harvard. \;\textbf{Imbens and Rubin's causal-inference class.} \;Students include Rajeev Dehejia, Sadek Wahba, Bruce Sacerdote, (later) David Autor.

\vspace{0.5em}
\begin{itemize}\setlength\itemsep{3pt}
  \item Rosenbaum--Rubin (1983) introduced the propensity score. \;\textit{LaLonde didn't use it.}
  \item Dehejia \& Wahba revisit the same data with \highlight{propensity-score matching}. \;They restrict to a sample with \textit{two years} of pre-treatment earnings.
  \item On that sample they recover \highlight{$\$1{,}794$} --- very close to the experimental ATT.
\end{itemize}
...
%% ---------- LL-Hist-8: Bridge to the walkthrough ----------
\begin{frame}{A closing example: what does conditional PT actually buy you?}
\vspace{0.5em}
We've spent the afternoon \textit{assuming} conditional PT is more credible than unconditional PT.
\vspace{0.5em}
\begin{itemize}\setlength\itemsep{3pt}
  \item Is it?
  \item Is there \textit{empirical} evidence that conditioning on $X$ recovers a known truth?
\end{itemize}
\vspace{0.6em}
\highlight{Yes. The canonical case is LaLonde.} \;We'll now run four observational specifications on the non-experimental panel and see which ones recover $\$1{,}794$.
\end{frame}

%% ---------- LL-2 (old name kept): LaLonde data, the panel ----------
\begin{frame}{The LaLonde panel}
\vspace{0.5em}
\textbf{Data structure} (Dehejia \& Wahba's two-year-pre version):
\vspace{0.5em}
\begin{itemize}\setlength\itemsep{3pt}
  \item Random assignment to NSW job training. Outcome: real 1978 earnings.
  \item Years on record: \texttt{74}, \texttt{75}, \texttt{78}.
\end{itemize}
\vspace{0.5em}
\meta{The experimental panel gives the truth. \;The non-experimental panel (NSW treated + CPS controls) is our test case.}
\end{frame}

%% ---------- LL-3: The benchmark ----------
\begin{frame}{The benchmark: \$1,794}
\vspace{1em}
\begin{center}
\Large
$\widehat{\text{ATT}}^{\text{RCT}} \;=\; \overline{Y}^{T}_{78} - \overline{Y}^{C}_{78} \;=\; 6{,}349 - 4{,}555 \;=\; \$1{,}794$
\end{center}
\vspace{1em}
\begin{center}
\small\color{warmgray}\itshape
...
%% ---------- LL-4: The challenge ----------
\begin{frame}{The challenge: swap the experimental controls for CPS}
\vspace{0.4em}
\textbf{Build a non-experimental panel.}
\vspace{0.4em}
\begin{itemize}\setlength\itemsep{3pt}
  \item Keep the NSW \textit{treated} group.
  \item Replace the experimental controls with a random sample of CPS households.
  \item Selection into NSW is sharply negative --- trainees had lower pre-earnings than the CPS pool.
\end{itemize}
\vspace{0.6em}
\highlight{Can observational DiD recover \$1,794 on this panel?}
\end{frame}

%% ---------- LL-5: Spec 0 — the equation ----------
\begin{frame}{Spec 0: naive DiD, no covariates}
\vspace{1.2em}
\begin{center}
\Large
$Y_{it} \;=\; \alpha \;+\; \beta_1\,\text{treat}_i \;+\; \beta_2\,\text{post}_t \;+\; \textcolor{accent}{\tau}\,(\text{treat}_i \times \text{post}_t) \;+\; \varepsilon_{it}$
\end{center}
\vspace{1em}
\begin{center}
\small\color{warmgray}\itshape
No $X$ anywhere. \;$\widehat{\tau}$ is the DiD coefficient on the interaction.
\end{center}
\end{frame}

%% ---------- LL-6: Spec 0 — Stata ----------
\begin{frame}[fragile]{Spec 0 in Stata}
\vspace{0.8em}
...
%% ---------- LL-7: Spec 0 result ----------
\begin{frame}{Spec 0 result: \$3,522 --- way off}
\vspace{1em}
\begin{center}
...
\midrule
RCT benchmark         & $\$1{,}794$ \\
\textbf{Spec 0} (no covariates) & \highlight{$\$3{,}522$} \\
\midrule
Gap                    & $+\$1{,}728$ \\
\bottomrule
\end{tabular}
...
%% ---------- LL-8: Spec A — the equation ----------
\begin{frame}{Spec A: add the $X$s as additive controls}
\vspace{0.6em}
\begin{center}
$Y_{it} \;=\; \alpha + \beta_1\,\text{treat}_i + \beta_2\,\text{post}_t + \textcolor{accent}{\tau}\,(\text{treat}_i {\times} \text{post}_t) \;+\; X_i'\,\gamma \;+\; \varepsilon_{it}$
\end{center}

\vspace{0.6em}
\textbf{Twelve covariates}, the standard LaLonde set:
\begin{center}
\small\ttfamily age, agesq, educ, educsq, marr, nodegree, black, hisp, re74, re75, u74, u75
\end{center}
...
%% ---------- LL-9: Spec A in Stata ----------
\begin{frame}[fragile]{Spec A in Stata}
\vspace{0.6em}
\begin{lstlisting}[language={[ANSI]C},basicstyle=\ttfamily\scriptsize]
...
%% ---------- LL-10: Spec A in R ----------
\begin{frame}[fragile]{Spec A in R}
\vspace{0.6em}
\begin{lstlisting}[language=R,basicstyle=\ttfamily\scriptsize]
library(fixest)
...
%% ---------- LL-11: Spec A in Python ----------
\begin{frame}[fragile]{Spec A in Python}
\vspace{0.6em}
...
%% ---------- LL-12: Spec A result ----------
\begin{frame}{Spec A result: \$3,522 --- \textit{identical} to Spec 0?!}
\vspace{0.6em}
\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}lr@{}}
...
\textbf{Spec A} (additive $X$) & \highlight{$\$3{,}522$} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.8em}
\textbf{Why?} \;Most of these $X$s are \textit{time-invariant} within a person --- age (within a window), education, race, pre-earnings.

\vspace{0.4em}
\highlight{Additive $X$ on a panel with time-invariant covariates does nothing} for the DiD coefficient. \;You need to let $X$'s effect \textit{move} in the post period.
\end{frame}

%% ---------- LL-13: Spec B — the equation ----------
\begin{frame}{Spec B: let $X$'s effect interact with post}
\vspace{0.7em}
\begin{center}
$Y_{it} = \alpha + \beta_1\,\text{treat}_i + \beta_2\,\text{post}_t + \textcolor{accent}{\tau}\,(\text{treat}_i{\times}\text{post}_t) \;+\; X_i'\gamma_1 \;+\; (\text{post}_t \cdot X_i)'\gamma_2 \;+\; \varepsilon_{it}$
\end{center}

\vspace{0.7em}
\begin{itemize}\setlength\itemsep{3pt}
  \item The $\gamma_1$ vector: $X$'s baseline effect (pre period).
  \item The $\gamma_2$ vector: how $X$'s effect \textit{shifts} in post.
\end{itemize}
...
%% ---------- LL-14: Spec B in Stata ----------
\begin{frame}[fragile]{Spec B in Stata}
\vspace{0.6em}
\begin{lstlisting}[language={[ANSI]C},basicstyle=\ttfamily\scriptsize]
global X "age agesq educ educsq marr nodegree black hisp re74 re75 u74 u75"
...
%% ---------- LL-15: Spec B in R ----------
\begin{frame}[fragile]{Spec B in R}
\vspace{0.6em}
\begin{lstlisting}[language=R,basicstyle=\ttfamily\scriptsize]
library(fixest)
...
%% ---------- LL-16: Spec B in Python ----------
\begin{frame}[fragile]{Spec B in Python}
\vspace{0.6em}
\begin{lstlisting}[language=Python,basicstyle=\ttfamily\scriptsize]
import statsmodels.formula.api as smf
...
%% ---------- LL-17: Spec B result ----------
\begin{frame}{Spec B result: \$1,184 --- much closer to \$1,794}
\vspace{0.5em}
\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}lr@{}}
...
\textbf{Spec B} ($\text{post} {\times} X$) & \highlight{$\$1{,}184$} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{center}
\small\color{warmgray}\itshape
Not exact. \;But the movement is \textit{toward} the truth, not away from it. \;From \$1,728 above to \$610 below.
\end{center}
\end{frame}

%% ---------- LL-Spec-C-1: Spec C — equation ----------
\begin{frame}{Spec C: heterogeneous $\tau$ in $X$ --- the saturated approach}
\footnotesize
\textbf{The motivation.} \;If the treatment effect itself depends on $X$ (e.g., training works better for younger workers), then \textit{no} single $\hat{\tau}$ coefficient is the ATT. \;You need to model the heterogeneity and average correctly.

\vspace{0.6em}
\textbf{The two-step regression-adjustment / OR estimator} (Heckman, Ichimura \& Todd, 1997):
\vspace{0.2em}
\begin{enumerate}\setlength\itemsep{2pt}
  \item Fit $\Delta Y \;=\; \alpha + X'\gamma + \nu$ on \textit{controls only}. \;(Models how $Y(0)$ trends as a function of $X$.)
  \item Predict the counterfactual $\widehat{\Delta Y_i}$ for each treated unit. \;Compute $\hat\tau_i = \Delta Y_i - \widehat{\Delta Y_i}$.
  \item $\widehat{\text{ATT}} = \tfrac{1}{N_{tr}}\sum_{i:D_i=1} \hat\tau_i$.
\end{enumerate}
...
%% ---------- LL-Spec-C-2: Spec C in Stata ----------
\begin{frame}[fragile]{Spec C in Stata}
\vspace{0.4em}
\begin{lstlisting}[language={[ANSI]C},basicstyle=\ttfamily\scriptsize]
...
%% ---------- LL-Spec-C-3: Spec C in R ----------
\begin{frame}[fragile]{Spec C in R}
\vspace{0.4em}
\begin{lstlisting}[language=R,basicstyle=\ttfamily\scriptsize]
...
%% ---------- LL-Spec-C-4: Spec C in Python ----------
\begin{frame}[fragile]{Spec C in Python}
\vspace{0.4em}
\begin{lstlisting}[language=Python,basicstyle=\ttfamily\scriptsize]
import statsmodels.formula.api as smf
...
%% ---------- LL-Spec-C-5: Spec C result ----------
\begin{frame}{Spec C result: \$1,263 --- closer still}
\vspace{0.5em}
\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}lr@{}}
...
\textbf{Spec C} (saturated OR / regression adjustment) & \highlight{$\$1{,}263$} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{center}
\small\color{warmgray}\itshape
Allowing $\tau$ to depend on $X$ moves the estimate further toward the truth --- but no regression estimator gets all the way. \;That's why propensity-score matching (Dehejia--Wahba) and DR-DiD (Sant'Anna--Zhao) exist: \\they put $\widehat{p}(X)$ in the right place.
\end{center}
\end{frame}

%% ---------- LL-18: Synthesis ----------
\begin{frame}{What this buys you: conditional PT is credible}
\vspace{0.5em}
\textbf{LaLonde's lesson, three decades on:}
\begin{itemize}\setlength\itemsep{3pt}
  \item Without covariates, observational DiD on the non-experimental sample is \highlight{wrong}.
  \item With covariates entered \textit{correctly} (interacted with post, or via matching / IPW / DR), it moves toward the truth.
  \item Dehejia \& Wahba (2002) and S\l{}oczy\'{n}ski (recent) recover \$1,794 closely on this same data using propensity-score methods.
\end{itemize}

\vspace{0.6em}
\highlight{Conditional PT is a real, defensible identifying assumption.} \;The empirical evidence is exactly this exercise --- and it's why the rest of the afternoon was worth the time.

\vspace{0.4em}
\meta{The lab: run the four covariate-aware estimators (TWFE-$X$, OR, IPW, DR-DiD) on the same data and see which one gets closest.}
\end{frame}
...
\end{itemize}
\vspace{0.3em}
\meta{Walkthrough: \texttt{glasgow/labs/Lalonde-Covariates/README.md}. \;Data: \texttt{lalonde\_exp\_panel.dta} \& \texttt{lalonde\_nonexp\_panel.dta} on Mixtape-Sessions GitHub.}
\end{frame}
