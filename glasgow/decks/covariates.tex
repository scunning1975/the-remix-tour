...
Every (pre-period) covariate gets $D{\times}X_{i,0}$, $\text{post}{\times}X_{i,0}$, and $D{\times}\text{post}{\times}X_{i,0}$ interactions. \;Then average the fitted $\tau(X_{i,0})$ over the treated $X$-distribution.

\vspace{0.5em}
\textbf{In principle this works. In practice it's a minefield:}
\begin{itemize}\setlength\itemsep{2pt}
  \item \textbf{True saturation discretizes $X$.} \;Continuous $X$ has to be binned. \;Rare cells $\to$ sparseness $\to$ same support failures IPW warns about.
  \item \textbf{Post-period $X$ is a bad control.} \;Use $X_{i,0}$, not $X_{it}$.
  \item \textbf{You still have to compute the ATT.} \;The headline coefficient at $X=0$ isn't it. \;Average $\hat\tau(X_{i,0})$ over the treated $X_{i,0}$-distribution.
\end{itemize}

\vspace{0.4em}
\highlight{OR, IPW, and DR do this semiparametrically.} \;Model $\widehat{\mu}_0(X)$ or $\widehat{p}(X)$ once; let the estimator do the saturation work and the ATT average for you.
\end{frame}
