...
\vspace{0.3cm}
\highlight{We will spend the first part of this lecture unpacking these weights, on the WTO/Lu-Yu (2015) data.}
\end{frame}
...
Distribution of $D_i$ = tariff reduction (in percentage points) across 155 Chinese manufacturing industries. \;\textbf{Mean dose} $\approx 7\%$. \;Min positive dose just above 0; max around 50\%.
\end{center}
\end{frame}
...
\begin{frame}{Imagine $\widehat\beta^{\text{twfe}}$ as a weighted average of dose-level effects}
\vspace{0.5cm}

CBS show that, under heterogeneous dose effects:
\[
\widehat\beta^{\text{twfe}} \;=\; \int_0^{\bar d} w(l)\,\cdot\,\text{ATT}(l \mid l)\,\,dl
\]

\vspace{0.4cm}

\textbf{Four different weights} $w(l)$ can be motivated:
\begin{itemize}\small\setlength\itemsep{1pt}
  \item $w^{\text{lev}}(l)$ \;--- the ``level'' weight implicit in OLS
  \item $w^{s}(l) = l \cdot w^{\text{lev}}(l)$ \;--- the scaled level
  \item $w^{\text{acrt}}(l)$ \;--- the weight on ATT($l \mid l$) you'd want for an ACRT interpretation
  \item $w^{2\times 2}(l, h)$ \;--- the weight on $(l, h)$ pairs of doses
\end{itemize}

\vspace{0.3em}
\highlight{Each one is a real, derivable answer to ``what is $\beta^{\text{twfe}}$?''} \;They give different numbers and different shapes.
\end{frame}
...
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.78\textwidth,height=0.70\textheight,keepaspectratio]{figures/dxt/06_conditional_mean.pdf}
\end{center}
\vspace{-0.05cm}
\begin{center}
\footnotesize\color{warmgray}\itshape
...
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.78\textwidth,height=0.70\textheight,keepaspectratio]{figures/dxt/07_weight_level.pdf}
\end{center}
\vspace{-0.05cm}
\begin{center}
\footnotesize\color{warmgray}\itshape
The level weight \textit{flips sign at $l = E[D]$}. \;Below-mean doses get negative weight; above-mean doses get positive weight.
\end{center}
\end{frame}

\begin{frame}{Weight \#2: the scaled-level weight $w^{s}(l) = l \cdot w^{\text{lev}}(l)$}
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.78\textwidth,height=0.70\textheight,keepaspectratio]{figures/dxt/08_weight_scaled_level.pdf}
\end{center}
\vspace{-0.05cm}
\begin{center}
\footnotesize\color{warmgray}\itshape
...
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.78\textwidth,height=0.70\textheight,keepaspectratio]{figures/dxt/09_weight_acrt.pdf}
\end{center}
\vspace{-0.05cm}
\begin{center}
\footnotesize\color{warmgray}\itshape
...
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.78\textwidth,height=0.70\textheight,keepaspectratio]{figures/dxt/10_weight_2x2_heatmap.pdf}
\end{center}
\vspace{-0.05cm}
\begin{center}
\footnotesize\color{warmgray}\itshape
Heatmap over \textit{pairs} of doses $(l, h)$. \;Far-apart pairs get the most weight --- $(h - l)^2$ shows up visually.
\end{center}
\end{frame}
...
\begin{center}
\footnotesize\color{warmgray}\itshape
Four overlaid weights, same distribution. \;$\widehat\beta^{\text{twfe}}$ doesn't pick one --- it is each of them simultaneously, depending on which decomposition you write down.
\end{center}
\end{frame}
...
\vspace{0.3em}
\meta{This is a function of $d$ --- one ATT per dose level. \;The full set $\{\text{ATT}(d \mid d)\}$ is the dose-response curve.}
\end{frame}
...
\textbf{A slope, not a level.} \;A dose-response curve: mg per mg of aspirin, km per km of distance, percentage point per percentage point of tariff cut.

\vspace{0.2em}
\meta{Angrist \& Imbens (1995) defined the analog for IV: the Average Causal Response (ACR).}
\end{frame}
...
\begin{center}
\footnotesize\color{warmgray}\itshape
$\text{ATT}(d \mid d)$: \;level effect of being at dose $d$. \;\;ACRT($d$): \;marginal effect of one more unit at dose $d$. \;Different objects, often confused.
\end{center}
\end{frame}
...
\highlight{Strong PT rules out selection on dose-response gains.} \;Exactly what an empiricist usually worries about.

\vspace{0.2em}
\meta{Randomization of dose gives strong PT automatically; selection on the dose-response curve violates it.}
\end{frame}

\begin{frame}{Kyle's running example: the discrete multi-valued case}
\vspace{0.6cm}

To build intuition, drop continuous for a moment and consider a \textit{multi-valued} discrete dose: $D \in \{0, 1, 2\}$.

\vspace{0.4cm}

\textbf{Setting.} \;A doctor has patients, some untreated ($D = 0$), some receiving one pill ($D = 1$), some two pills ($D = 2$). \;The discrete case is the bridge between binary and continuous --- all the same ideas, with finite groups.

\vspace{0.4cm}

\textbf{Next slides.} \;What dose-by-dose ATTs look like; first-differences; the DiD; what's actually identified.

\vspace{0.4em}
\meta{From Kyle Butts' continuous DiD slides.}
\end{frame}
...
\highlight{Part 3 estimates these objects, using Kyle's three options: overall ATT, bins, and non-parametric smoothing.}
\end{frame}

% =============================================================================
% PART IV — Estimation (Kyle Butts material)
% =============================================================================
...
Kyle Butts's R package \texttt{contdid} implements all three options.
