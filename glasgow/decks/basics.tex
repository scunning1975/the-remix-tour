...
%% ---------- Slide 28h4: The weighted mean — formula ----------
\begin{frame}{The weighted mean --- the formula behind ``weighting decides''}
\footnotesize
\textbf{Weighted average.} \;Given values $Y_1, \ldots, Y_n$ with weights $w_1, \ldots, w_n$:
\vspace{-0.2em}
\[
\overline{Y}_w \;=\; \frac{w_1 Y_1 + w_2 Y_2 + \cdots + w_n Y_n}{w_1 + w_2 + \cdots + w_n}
\;=\; \frac{\sum_i w_i\,Y_i}{\sum_i w_i}.
\]

\textbf{Two special cases of the same formula.}
\begin{itemize}\setlength\itemsep{2pt}
  \item All weights equal ($w_i = 1$): \;collapses to the ordinary average \;$\overline{Y} = \frac{1}{n}\sum_i Y_i$ \;\textit{(unit-weighted)}.
  \item Weights = cell counts (population, persons-per-county): \;\textit{person-weighted}.
\end{itemize}

\vspace{0.4em}
\textbf{Same operation, three syntaxes.}
\begin{itemize}\setlength\itemsep{2pt}
  \item \textbf{Stata:} \;\texttt{reg y x [aweight=pop]}
  \item \textbf{R:} \;\texttt{lm(y \textasciitilde\ x, weights = pop)} \,or\, \texttt{feols(y \textasciitilde\ x, weights = \textasciitilde pop)}
  \item \textbf{Python:} \;\texttt{smf.wls('y \textasciitilde\ x', data=df, weights=df.pop).fit()}
\end{itemize}

\vspace{0.2em}
\meta{This is also the formula behind ``two ATTs, two parallel trends'' later --- weighted least squares with population weights.}
\end{frame}

%% ---------- Slide 28h5: Two DiDs on the same 10 people ----------
\begin{frame}{Apply it: two DiDs on the same 10 people}
\footnotesize
The earlier 10-person table. \;Compute a DiD for each county separately:
\begin{itemize}\setlength\itemsep{1pt}
  \item \textbf{County 1} (8 people): \;DiD$_1 = +0.5$
  \item \textbf{County 2} (2 people): \;DiD$_2 = -1.0$
\end{itemize}

\vspace{0.5em}
\textbf{Person-weighted DiD} (weight by population):
\vspace{-0.2em}
\[
\widehat{\text{DiD}}_{\text{pop}} \;=\; \frac{8 \cdot (+0.5) \;+\; 2 \cdot (-1.0)}{8 + 2} \;=\; \frac{4 - 2}{10} \;=\; \mathbf{+0.2}
\]

\vspace{0.2em}
\textbf{Unit-weighted DiD} (one weight per county):
\vspace{-0.2em}
\[
\widehat{\text{DiD}}_{\text{unit}} \;=\; \frac{(+0.5) + (-1.0)}{2} \;=\; \mathbf{-0.25}
\]

\vspace{0.4em}
\begin{center}
\small\color{warmgray}\itshape
Same data. \;Same county DiDs. \;\highlight{Different signs} when you aggregate. \;The weight is the choice.
\end{center}
\end{frame}

%% ---------- Slide 28h6: Castle data, weighted — Stata ----------
\begin{frame}[fragile]{Castle data, weighted by population: Stata}
\footnotesize
\textbf{Manual weighted 2$\times$2.} \;Four cell means, each with \texttt{aw=popwt}:
\vspace{-0.2em}
...
\vspace{0.3em}
\textbf{Same number via regression:}
\vspace{-0.2em}
\begin{lstlisting}[language={[ANSI]C},basicstyle=\ttfamily\scriptsize]
reg l_homicide post##treat [aweight=popwt], cluster(sid)
// Coefficient on 1.post#1.treat = 0.0964
\end{lstlisting}

\vspace{0.2em}
\meta{Same number to machine epsilon. \;\texttt{aweight} \textit{is} the formula on slide 28h4 --- applied inside OLS.}
\end{frame}

%% ---------- Slide 28h7: Castle data, weighted — R ----------
\begin{frame}[fragile]{Same in R}
\vspace{0.4em}
\begin{lstlisting}[language=R,basicstyle=\ttfamily\scriptsize]
library(haven); library(dplyr); library(fixest)
castle <- read_dta("castle.dta")

# Manual: weighted means by cell, then (Y11-Y10)-(Y01-Y00)
castle |>
  group_by(treat, post) |>
  summarize(Ybar = weighted.mean(l_homicide, popwt),
            .groups = "drop")
# (wy11 - wy10) - (wy01 - wy00)  ->  0.0964

# Regression with population weights, clustered SEs
feols(l_homicide ~ post * treat, data = castle,
      weights = ~popwt, cluster = ~sid)
# Coefficient on post:treat = 0.0964
\end{lstlisting}
\end{frame}

%% ---------- Slide 28h8: Castle data, weighted — Python ----------
\begin{frame}[fragile]{Same in Python}
\vspace{0.4em}
...
# Manual: weighted means, four cells
def wmean(df):
    return (df.l_homicide * df.popwt).sum() / df.popwt.sum()
means = castle.groupby(["treat", "post"]).apply(wmean)
# (wy11 - wy10) - (wy01 - wy00)  ->  0.0964

# Regression with population weights, clustered SEs
mod = smf.wls("l_homicide ~ post * treat", data=castle,
              weights=castle["popwt"]).fit(
        cov_type="cluster", cov_kwds={"groups": castle["sid"]})
print(mod.params["post:treat"])    # 0.0964
\end{lstlisting}
\end{frame}

%% ---------- Slide 29: From parameters to the target parameter ----------
\begin{frame}{The switching equation --- and a target parameter}
\footnotesize
\textbf{Everything until now has been about \textit{parameters}.}
\begin{itemize}\setlength\itemsep{1pt}
  \item ATE, ATT, ATU --- three parameters, three weights, sometimes three signs.
  \item Whichever you report is a choice about \textit{which causal effect you actually want}.
\end{itemize}

\vspace{0.4em}
\textbf{Pick one: today's target parameter is the ATT.} \;The average effect for the units that actually received the treatment.

\vspace{0.6em}
\textbf{But to estimate \textit{any} of these parameters from data}, you confront:
\vspace{-0.2em}
\[
...
\vspace{-0.7em}
\begin{itemize}\setlength\itemsep{1pt}
  \item The \highlight{switching equation}: for each unit you see one half, never both.
  \item All three parameters ask about $Y(1)$ \textit{and} $Y(0)$ together. \;The data hands you one column.
\end{itemize}

\vspace{0.3em}
\meta{The rest of the day is strategies for recovering the ATT from the realized half. \;DiD is the first one.}
\end{frame}
...
\vspace{0.4em}
\begin{itemize}
  \item Unit FE $\alpha_i$, time FE $\lambda_t$
  \item One indicator per event-time, treated only
  \item \highlight{Drop $k=-1$ as the reference} --- every other $\hat{\beta}_k$ is a 2$\times$2 against the period just before treatment
\end{itemize}
\end{frame}

%% ---------- Slide 52: t-1 is the hinge ----------
\begin{frame}{The baseline is the hinge}
\vspace{0.4em}
\begin{center}
\begin{tikzpicture}[scale=0.95]
\draw[->, slate, thick] (-4.5,0) -- (4.5,0) node[right, text=charcoal, font=\small] {event time};

%% tick marks
\foreach \x/\lab in {-3/-3, -2/-2, -1/-1, 0/0, 1/+1, 2/+2, 3/+3} {
  \draw[slate] (\x,-0.1) -- (\x,0.1);
  \node[font=\scriptsize, text=charcoal, anchor=north] at (\x,-0.15) {\lab};
}

%% Highlight t=-1
\fill[accent] (-1,0) circle (0.12);
\node[font=\scriptsize\bfseries, text=accent, anchor=south] at (-1,0.2) {baseline};

%% Treatment line
\draw[dashed, accent, thick] (0,-0.5) -- (0,1.5);
\node[font=\scriptsize, text=accent, anchor=south] at (0,1.5) {treatment};

%% Coefficient dots
\foreach \x/\y in {-3/0.6, -2/0.4, 0/1.0, 1/1.3, 2/1.4, 3/1.4} {
  \fill[charcoal] (\x,\y) circle (0.08);
}
%% Reference (zero) at t=-1
\node[font=\scriptsize, text=charcoal, anchor=south] at (3.5,1.45) {$\hat\beta_k$};
...
\begin{center}
\small\color{warmgray}\itshape
Each $\hat\beta_k$ is a 2$\times$2 DiD with pre $= -1$, post $= k$: \\[0.15em]
$\big(\overline{Y}^T_k - \overline{Y}^T_{-1}\big) - \big(\overline{Y}^C_k - \overline{Y}^C_{-1}\big)$. \;\; The hinge is at $-1$, not in the levels.
\end{center}
\end{frame}
