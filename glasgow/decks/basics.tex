...
\definecolor{breadcolor}{HTML}{2B6CB0}   % ocean blue — the bread is always (X'X)^{-1}
\definecolor{meatcolor}{HTML}{A51C30}    % crimson    — the meat is the error-structure middle
...
\vspace{-0.4em}
\footnotesize
By the late 1970s, empirical labor economics was in trouble. Different methods on the same question \textit{disagreed wildly}. Two pieces crystallized it:
\begin{itemize}
...
%% ---------- Slide 38c4: Case 3 — control receives treatment too ----------
\begin{frame}{Case 3.\ The control group also receives treatment in Post}
\scriptsize
The comparison group's Post outcome is also $Y(1)$, not $Y(0)$. \;Switching equation:
\vspace{-0.3em}
\[
...
\textbf{Add two zeros} --- one missing counterfactual on each side:
\vspace{-0.2em}
\[
+\,\mathbb{E}[Y(0)|1,\text{Post}] - \mathbb{E}[Y(0)|1,\text{Post}] \quad\text{and}\quad +\,\mathbb{E}[Y(0)|0,\text{Post}] - \mathbb{E}[Y(0)|0,\text{Post}]
\]
\vspace{-1.0em}

\textbf{Rearrange.} \;Treated side $\to$ ATT $+$ treated $Y(0)$ trend. \;Control side $\to$ a \textit{control-group} ATT $+$ control $Y(0)$ trend.
\vspace{-0.2em}
\[
\widehat{\text{DiD}} \;=\; \underbrace{\mathbb{E}[Y(1){-}Y(0)|1,\text{Post}]}_{\textstyle\text{\color{accent}\bfseries ATT}}
   \;+\; \underbrace{\big\{\Delta\mathbb{E}[Y(0)|1] - \Delta\mathbb{E}[Y(0)|0]\big\}}_{\text{non-PT bias}\,=\,0\text{ under PT}}
   \;-\; \underbrace{\mathbb{E}[Y(1){-}Y(0)|0,\text{Post}]}_{\textstyle\text{\color{accent}\bfseries ATT}_{D{=}0,\text{Post}}\;\text{\rmfamily\itshape (the spillover)}}
\]
\vspace{-0.6em}
...
%% ---------- Slide 38c4b: Case 3 — commentary on the decomposition ----------
\begin{frame}{Case 3, in shorthand}
\footnotesize
The decomposition from the previous slide, written compactly:

\vspace{0.6em}
\begin{center}
\Large
$\widehat{\text{DiD}} \;=\; (+1)\cdot \text{ATT}_{\text{Post}} \;+\; (+1)\cdot\text{PTA} \;+\; (-1)\cdot\text{ATT}_{\text{Post},\,D=0}$
\end{center}

\vspace{0.6em}
\textbf{The weights sum to one:} $\;+1 + 1 - 1 = 1$. \;But one weight is \highlight{negative}.

\vspace{0.6em}
\begin{itemize}\setlength\itemsep{2pt}
  \item A clean weighted average of ATTs has \textit{all positive} weights.
  \item This isn't that. \;The spillover ATT enters with a $-1$.
  \item Under PT (bias $=0$), DiD is the treated ATT \textit{minus} the control ATT.
\end{itemize}

\vspace{0.3em}
\meta{Memorize the formula. Work the algebra. Watch for negative weights --- they show up again in staggered DiD.}
\end{frame}
...
%% ---------- Slide 38c5b: The Monte Carlo program ----------
\begin{frame}[fragile]{The Monte Carlo, in one R script}
\footnotesize
\textbf{Where it lives:}
\vspace{-0.2em}
\begin{lstlisting}[language=bash,basicstyle=\ttfamily\scriptsize]
decks/code/case_decompositions.R
\end{lstlisting}

\vspace{0.5em}
\textbf{What it does:}
\begin{itemize}\setlength\itemsep{2pt}
  \item Generates treatment effects for every unit (true ATT $= 10$).
  \item Simulates four DGPs --- one per case from the last four slides.
  \item Repeats 2000 times. \;Records $\widehat{\text{DiD}}$ each time.
  \item Plots the sampling distribution of $\widehat{\text{DiD}}$ for each case.
\end{itemize}

\vspace{0.5em}
\begin{center}
\small\color{warmgray}\itshape If the algebra was right, the simulation lands where the algebra said.
\end{center}
\end{frame}
...
%% ---------- Slide 39a: Design vs. model — where PT lives ----------
\begin{frame}{Parallel trends: a different \textit{kind} of assumption}
\footnotesize
\textbf{Randomization is a design.} \;The parent process \textit{guarantees} full distributional equivalence between treatment groups. \;In the population, $\mathbb{E}[Y(1)|D{=}0] = \mathbb{E}[Y(1)|D{=}1]$. \;Plug-ins identify the ATE.

\vspace{0.6em}
\textbf{Parallel trends is a model.} \;An asserted $Y(0)$ trend. \;Sibling to RDD smoothness and synthetic control's factor model. \;\highlight{No parent process backs it.}

\vspace{0.7em}
\textit{Stronger? Weaker? \;Different kind of thing.}
\begin{itemize}\setlength\itemsep{3pt}
  \item \textbf{Weaker}: you only need equal $Y(0)$ \textit{trends}, not full distributional equivalence.
  \item \textbf{Stronger}: you have to defend the assertion with no design that enforces it.
\end{itemize}

\vspace{0.4em}
\meta{Pre-trends and falsifications are heuristics, not identification --- they're finger-in-the-air checks for whether the asserted model looks reasonable.}
\end{frame}

%% ---------- Slide 39a2: The model-based family ----------
\begin{frame}{The model-based family of identification}
\footnotesize
\textbf{Three siblings.} \;Each asserts something about $Y(0)$ that the data cannot directly confirm.

\vspace{0.7em}
\begin{itemize}\setlength\itemsep{5pt}
  \item \textbf{Parallel trends} (DiD): \;$\mathbb{E}[Y(0) \mid D{=}1]$ \textit{trends} like $\mathbb{E}[Y(0) \mid D{=}0]$ in the post period.
  \item \textbf{Smoothness} (RDD): \;$\mathbb{E}[Y(0) \mid X{=}x]$ is continuous as $x$ crosses the threshold.
  \item \textbf{Factor model} (synthetic control): \;$Y(0)_{it}$ follows a low-rank structure the donor pool spans.
\end{itemize}

\vspace{0.7em}
\highlight{None of these are designed.} \;You assert them, then check that the asserted shape is consistent with what you can observe. \;The assumption itself lives in the counterfactual --- where the data is silent.
\end{frame}

%% ---------- Slide 39a3: What backs a model-based assumption ----------
\begin{frame}{What backs a model-based assumption}
\footnotesize
\textbf{Randomization is its own defense.} \;The lottery was fair; the assumption holds by construction.

\vspace{0.5em}
\textbf{A model-based assumption has no such defense.} \;What backs it instead:

\vspace{0.5em}
\begin{itemize}\setlength\itemsep{4pt}
  \item \textbf{Institutional reasoning.} \;Why would treated and control move together absent treatment? \;What forces shaped both? \;Where does the variation in treatment come from?
  \item \textbf{Choice of comparison group.} \;Closer in covariates, exposure, and history $\to$ more credible.
  \item \textbf{Pre-period anchoring.} \;A long stable pre-period of parallel movement makes the asserted post-period trend easier to believe.
\end{itemize}

\vspace{0.4em}
\meta{The most credible DiDs (Card-Krueger, Mariel) earn their credibility from the institutional story --- not from the pre-trend test.}
\end{frame}

%% ---------- Slide 39b: Parallel trends, drawn ----------
...
\vspace{-0.2em}
\[
\text{Var}(\hat\beta) \;=\; \underbrace{\bread{(X'X)^{-1}}}_{\textstyle \bread{\text{bread}}}\;\; \underbrace{\meat{X'\,\text{Var}(\varepsilon \mid X)\,X}}_{\textstyle \meat{\text{meat}}} \;\; \underbrace{\bread{(X'X)^{-1}}}_{\textstyle \bread{\text{bread}}}.
\]
\vspace{0.0em}
\begin{itemize}\small
  \item The \bread{\textbf{bread}} $\bread{(X'X)^{-1}}$ comes from OLS mechanics. We always have it.
  \item The \meat{\textbf{meat}} $\meat{X' \text{Var}(\varepsilon \mid X)\, X}$ is where the \textit{error structure} lives.
  \item All three flavors (vanilla, robust, cluster-robust) have the same \bread{bread}. \\ \highlight{They differ only in how they estimate the \meat{meat}.}
\end{itemize}
\vspace{0.4em}
\begin{center}
\small\color{warmgray}\itshape
...
\vspace{-0.2em}
\[
\text{Var}(\hat\beta) \;=\; \bread{(X'X)^{-1}}\, \meat{\big(X' \,\text{Var}(\varepsilon \mid X)\, X\big)}\, \bread{(X'X)^{-1}}
\]
\vspace{-0.2em}

Under homoskedasticity $\text{Var}(\varepsilon \mid X) = \sigma^2 \mathbf{I}$, so the \meat{meat} becomes $\meat{\sigma^2 (X'X)}$, and the sandwich collapses:
\vspace{-0.2em}
\[
\text{Var}(\hat\beta) \;=\; \bread{(X'X)^{-1}}\, \meat{\sigma^2(X'X)}\, \bread{(X'X)^{-1}} \;=\; \meat{\sigma^2}\, \bread{(X'X)^{-1}}.
\]
\vspace{0.1em}
\begin{itemize}\small
...
\vspace{0.5em}
Now $\sigma_i^2 = \text{Var}(\varepsilon_i \mid X_i)$ varies with $i$. The \meat{meat} is:
\vspace{-0.2em}
\[
\meat{X'\, \text{diag}(\sigma_1^2, \,\sigma_2^2, \,\dots,\, \sigma_n^2)\, X} \;\;\neq\;\; \meat{\sigma^2 (X'X)}.
\]
\vspace{-0.2em}
\begin{itemize}\small
...
\vspace{-0.2em}
\[
\widehat{\text{Var}}_{\text{HC}}(\hat\beta) \;=\; \bread{(X'X)^{-1}}\, \meat{\bigg(\sum_i \hat e_i^2\, X_i X_i'\bigg)}\, \bread{(X'X)^{-1}}.
\]
\vspace{0.1em}
\begin{itemize}\small
...
\vspace{0.4em}
\[
\widehat{\text{Var}}_{\text{CRVE}}(\hat\beta) \;\propto\; \bread{(X'X)^{-1}}\!\left(\meat{\sum_g s_g s_g'}\right)\!\bread{(X'X)^{-1}}
\]
\vspace{0.2em}
\begin{itemize}
...
With $G=3$, $n=6$, $k=2$, and $S_{xx} = \sum_i \tilde x_i^2 = (-2)^2{+}(-2)^2{+}0{+}0{+}2^2{+}2^2 = \bread{16}$, so $\bread{S_{xx}^2 = 256}$:
\[
...
$\dfrac{n}{n-k}\cdot \dfrac{\meat{\sum_i \tilde x_i^2 \hat e_i^2}}{\bread{\big(\sum_i \tilde x_i^2\big)^2}}$
&
$\dfrac{G}{G-1}\cdot \dfrac{n-1}{n-k}\cdot \dfrac{\meat{\sum_g s_g^2}}{\bread{\big(\sum_i \tilde x_i^2\big)^2}}$ \\[1.4em]
...
\begin{itemize}\small
  ...
  \item Matrix form: same sandwich $\bread{(X'X)^{-1}}\big(\meat{\text{meat}}\big)\bread{(X'X)^{-1}}$, but the \meat{meat} differs.
\end{itemize}
\vspace{0.2em}
\meta{Same shape --- \bread{bread}, \meat{meat}, \bread{bread}. \;Difference: whether the \meat{meat} sums over obs or over clusters.}
\end{frame}
...
%% ---------- Slide 51f: Implication for pre-trends test ----------
\begin{frame}{Why the hinge matters for the pre-trends test}
\vspace{0.4em}
...
\begin{itemize}\setlength\itemsep{4pt}
  ...
  \item A \highlight{pre-trends check} asks whether those long differences are statistically zero.
  \item Post-period coefficients use the \textit{same hinge}. \;The pre-period model has to match the long-difference structure of the post-period model.
\end{itemize}
...
%% ---------- Slide 51g: Pre-trends are NOT parallel trends ----------
\begin{frame}{Pre-trends are not parallel trends}
\footnotesize
\textbf{The parallel-trends assumption} (post-period, counterfactual):
\vspace{-0.3em}
\[
\big\{ \mathbb{E}[Y(0)|D{=}1, t] - \mathbb{E}[Y(0)|D{=}1, t{-}1] \big\} \;-\; \big\{ \mathbb{E}[Y(0)|D{=}0, t] - \mathbb{E}[Y(0)|D{=}0, t{-}1] \big\} \;=\; 0
\]

\vspace{-0.4em}
\textbf{Parallel \textit{pre}-trends} (pre-period, observable):
\vspace{-0.3em}
\[
\big\{ \mathbb{E}[Y(0)|D{=}1, t{-}2] - \mathbb{E}[Y(0)|D{=}1, t{-}1] \big\} \;-\; \big\{ \mathbb{E}[Y(0)|D{=}0, t{-}2] - \mathbb{E}[Y(0)|D{=}0, t{-}1] \big\} \;=\; 0
\]

\vspace{0.4em}
\begin{itemize}\setlength\itemsep{3pt}
  \item Different periods. \;Different conditional expectations. \;\highlight{Different quantities.}
  \item Neither implies the other. \;Pre-trends can be flat and post-period PT can fail. \;Pre-trends can wobble and post-period PT can still hold.
  \item Flat pre-trends are \textbf{heuristic evidence}, not a test of the identifying assumption.
\end{itemize}
\end{frame}
...
%% =====================================================================
\remixsection{7.\ Two ATTs, Two PTs \\[0.3em] {\normalsize\normalfont A callback to weighting}}
...
%% ---------- W5b: Case A event study — covgap = 0, both PTs hold ----------
\begin{frame}{Case A: covgap $\approx$ 0 --- both pre-trends look flat}
\vspace{0.2em}
\begin{center}
\includegraphics[width=0.96\linewidth,height=0.68\textheight,keepaspectratio]{figures/weighted_pt_eventstudy_A.pdf}
\end{center}
\vspace{-0.3em}
\begin{center}
\footnotesize\color{warmgray}\itshape
Same data. \;Both event studies look clean. \;\highlight{Different post estimates --- because they target different ATTs.}
\end{center}
\end{frame}

%% ---------- W5c: Case B event study — covgap large, weighted PT fails ----------
\begin{frame}{Case B: covgap large --- the weighted event study breaks}
\vspace{0.2em}
\begin{center}
\includegraphics[width=0.96\linewidth,height=0.68\textheight,keepaspectratio]{figures/weighted_pt_eventstudy_B.pdf}
\end{center}
\vspace{-0.3em}
\begin{center}
\footnotesize\color{warmgray}\itshape
Selection on size shifts $\kappa_1 - \kappa_0$ away from zero. \;The unweighted event study still passes; the weighted one drifts. \;\highlight{The pre-trend test belongs to the assumption you're running.}
\end{center}
\end{frame}
...
\small
Event studies (we just saw) interrogate \textit{which} PT you can defend. You can run them weighted, unweighted, or both --- but each tests a different assumption.
\end{block}
\vspace{0.2em}
...
%% MOVED to the Covariates deck (Day 2 afternoon). Commented out here
%% so the lab gets introduced in the right session.
