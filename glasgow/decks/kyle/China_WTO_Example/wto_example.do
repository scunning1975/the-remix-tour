...
clear all
global root "."
use "${root}/data/industry_by_year.dta", clear
...
predict te_est_linear if dose > 0
...
* Get estimates for plotting
predict te_est_spline if dose > 0
...
gen bin_3 = dose >= 0.2
reg Delta_y_diff ib0.bin_1 ib0.bin_2 ib0.bin_3 if dose > 0, nocons

* Get estimates for plotting
predict te_est_bins if dose > 0

* Plot
sort dose
twoway ///
  (scatter Delta_y_diff dose, mcolor(gs7)) ///
  (line te_est_linear dose) ///
  (line te_est_spline dose) ///
  (line te_est_bins dose if dose < 0.1, lcolor(blue)) ///
  (line te_est_bins dose if dose >= 0.1 & dose < 0.2, lcolor(blue)) ///
  (line te_est_bins dose if dose >= 0.2, lcolor(blue)), ///
  xlabel(, grid) ///
  ylabel(-1(0.5)1, grid) ///
