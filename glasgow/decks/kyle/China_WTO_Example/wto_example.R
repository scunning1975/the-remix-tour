...
df <- read_csv("China_WTO_Example/data/industry_by_year.csv")
...
collapsed <- df |>
  ...
  summarize(
    ...
    # treatment dose (bite of the policy)
...
# Regressing
# (Y_{i1} - Y_{i0}) - E( Y_{i1} - Y_{i0} | D_i = 0 ) on a linear function
est_linear <- feols(
...
## Get treatment effect estimates for plotting
