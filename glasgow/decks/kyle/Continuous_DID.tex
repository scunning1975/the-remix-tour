...
\imageframe{figures/discrete_raw_first_differences.pdf}
...
\imageframe{figures/discrete_implied_counterfactual.pdf}
\imageframe{figures/discrete_did_estimates.pdf}
...
\begin{frame}{``ATT'' in continuous DID}
  ...
  There are now three different potential outcomes
  $$
    Y_{i1}(d),
  $$
...
%       \begin{center}
%         \setstretch{0.8}
%         {\footnotesize\color{blue} Hypothetical dose}
%       \end{center}
%     };
%
...
\begin{frame}{Thinking about counterfactual treatment effects}
  ...
  % \devgrid
  \begin{tikzpicture}[remember picture, overlay]
    \node [anchor = south, text width = 0.4\textwidth] at (page cs:0.56,0.57) {
      \begin{center}
        \setstretch{0.8}
        {\footnotesize\color{blue} Effect of increasing dose by a little}
      \end{center}
    };

    \node [anchor = north, text width = 0.35\textwidth] at (page cs:0.77,0.62) {
      \begin{center}
        \setstretch{0.8}
...
\imageframe{figures/discrete_te_plot.pdf}
...
\imageframe{figures/discrete_te_linear_homogenous.pdf}
\imageframe{figures/discrete_te_linear_het.pdf}
\imageframe{figures/discrete_te_constant.pdf}
\imageframe{figures/discrete_te_maximizing.pdf}
...
\begin{frame}{Recap}
  ...
  \bigskip
  {\color{blue} 1.} Dose-group specific DID estimates identify the $\text{ATT}(d \ | \ d)$ effects

  \bigskip
  {\color{blue} 2.} Comparing across different doses does not inform us of the marginal effect of changing a dose, the $\text{ACR}(d \ | \ d)$
\end{frame}
...
\begin{frame}{``Strong Parallel Trends''}
  ...
  \bigskip
  The first asusmption is what allows estimation of $\text{ATT}(d \ | \ d)$ 
  \begin{itemize}
    \item DID of units with $D_i = d$ compared to $D_i = 0$ group 
    
  \end{itemize}
 
  \pause
  \bigskip
  The latter assumption is what let's us compare $\text{ATT}(d)$ across $d$ to identify the $\text{ACR}$ curve
  \begin{itemize}
    \item With continuous dosage, the slope of $\text{ATT}(d)$ is the $\text{ACR}$ curve
  \end{itemize}
\end{frame}
...
\imageframe{figures/continuous_raw_first_differences.pdf}
...
\imageframe{figures/continuous_did_estimates.pdf}
...
\begin{frame}{$\text{ATT}(d \ | \ d)$ estimators}
  In general, our $\text{ATT}(d \ | \ d)$ estimators will take the form of
  $$
    \Delta Y_i - \expechat{\Delta Y_i}{D_i = 0}
  $$
...
\begin{frame}{Option 1: Overall ATT}
  For all treated units, calculate
  $$
    \Delta Y_i - \expechat{\Delta Y_i}{D_i = 0}
  $$
  \begin{itemize}
    \item It is the \emph{observed} change in $Y$ minus the counterfactual (estimated) change in $Y(0)$
  \end{itemize}

  \bigskip
  Then, average $\Delta Y_i - \expechat{\Delta Y_i}{D_i = 0}$ for everyone with $D_i > 0$
  \begin{itemize}
    \item This approach is unsatisfying because we've effectively `binarized' our treatment into a simple DID setup
  \end{itemize}
\end{frame}


\begin{frame}{Option 2: Bins}
  Instead, we can use a classic trick of `discretizing' the dose into a set of bins
  \begin{itemize}
    \item Similar to using age bins instead of polynomial of age
  \end{itemize}

  \pause
  \bigskip
  Take $\Delta Y_i - \expechat{\Delta Y_i}{D_i = 0}$ and regression on indicators for each bin
\end{frame}

\imageframe{figures/continuous_binned_avgs.pdf}
...
\begin{frame}{Pros and Cons of Bins}
  ...
  \begin{itemize}
    ...
    \item Poor estimates of $ACRT(d)$ because the slope is 0 everywhere except the bin endpoints.
  \end{itemize}
\end{frame}
...
\imageframe{figures/continuous_smoothed_avgs.pdf}

\begin{frame}[fragile]{\texttt{contdid}}
  The `contdid' package by CGBS does this directly non-parametric smoothing method (trimmed source):
  % Source: https://github.com/bcallaway11/contdid/blob/443c1c12f013bdaf19503d9d0a4f7a494e9eb22f/R/cont_did.R#L189-L216
...
\begin{frame}{Pros and Cons of Non-parametric smoothing}
  ...
  \begin{itemize}
    ...
    \item Pre-trends are hopefully flat across doses 
    \begin{itemize}
      \item Only assesses half of the strong parallel trends assumption
    \end{itemize}

    \item The $\text{ACRT}(d)$ estimate is the derivative of the smoothed curve (can estimate using spline procedures)
  \end{itemize}
\end{frame}

\section{Linear Model of without ``Strong PTs''}

\begin{frame}{Linear in Dose}
  Consider the standard TWFE type model (written in first-differences):
  $$
  \Delta Y_i = \alpha + \tau D_i + u_i
  $$
  \begin{itemize}
    \item The unit-fixed effect cancels out
    
    \item $\alpha$ is the common time-trend (the $D_i = 0$ group will estimate)
    
    \item $D_i$ is assumed linear with common coefficient
  \end{itemize}
\end{frame}

\begin{frame}{OLS Coefficient}
  The (true) heterogeneous treatment effects are given by $\text{ATT}(D_i \ \vert \ D_i)$.

  \bigskip
  $$
    \Delta Y_i - \hat{\alpha} = \text{ATT}(D_i \ \vert \ D_i) + \text{noise} = \tau D_i + u_i
  $$
  
  \bigskip
  Our regression coefficient is the best linear predictor of the data $y = \text{ATT}(D_i \ \vert \ D_i) + \text{noise}$ and $x = D_i$
\end{frame}
...
\begin{frame}{OLS Coefficient}
  ...
  \bigskip 
  OLS compares units with larger to smaller doses. We might be \emph{really tempted} to call $\hat{\tau}$ the slope the dose-specific ATT curve
  When doing so, it does two things:
  \begin{enumerate}
    \item changes the dose received
    \item changes the group of treated units you're examining
  \end{enumerate}  
\end{frame}

\begin{frame}{Bias of coefficient for ACRT}
  $$
    \Delta Y_i - \hat{\alpha} = \text{ATT}(D_i \ \vert \ D_i) + \text{noise} = \tau D_i + u_i
  $$

  OLS compares units with larger to smaller doses. When doing so, it does two things:
  \begin{enumerate}
    \item changes the dose received
    \item \textbf{changes the group of treated units you're examining}
  \end{enumerate}

  \bigskip
  It is the second change that prevents us from estimating the effect of increasing the dose for units (marginal effect)
\end{frame}

\begin{frame}{Adding `Strong PTs'}
  $$
    \Delta Y_i - \hat{\alpha} = \text{ATT}(D_i) + \text{noise} = \tau D_i + u_i
  $$

  If we assume strong parallel trends, then $\hat{\tau}$ estimates a weighted average of the $ACR(d)$ curve
  \begin{itemize}
    \item Something like an ``average slope'' $\text{ATT}(D_i)$ 
  \end{itemize}
\end{frame}
...
\begin{frame}{Defining continuous treatment variable}
  ...
  \emph{Aside: Often reframing a continuous variable (Tariff) to changes (Change in Tariff) can help you have a $D_i = 0$ group}
\end{frame}
...
\imageframe{figures/china_wto_raw.pdf}
...
\imageframe{figures/china_wto_ests.pdf}
...
\imageframe{figures/china_wto_pretrends_ests.pdf}

\section*{Covariates}

\begin{frame}[fragile]{Adding covariate-specific trends}
  If we want to assume that units with the same value of $X_i = x$ are on the same counterfactual trend (regardless of their $D_i$), we can!

  \begin{enumerate}
    \item Estimate $\Delta Y_i = X_i \gamma + v_i$ using $D_i = 0$ group (or a more flexible function of $X_i$)
    
    \item Form $\Delta Y_i - X_i \hat{\gamma}$ where $X_i \hat{\gamma}$ is the estimated $\Delta Y_i(0)$ for the treated units
    
    \item Regress $\Delta Y_i - X_i \hat{\gamma}$ on your functions of $D_i$ (linear, bins, splines)
  \end{enumerate}
\end{frame}
