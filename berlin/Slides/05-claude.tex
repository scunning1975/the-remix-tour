%% 05-claude.tex — Sant'Anna Workshop Pisa: Claude Code for empirical research
...
%% ====================================================================
%% PART 2 — THE ECONOMICS OF AI (Shockley first, then isoquants)
%% ====================================================================

\begin{frame}{Research output is multiplicative}
\vspace{1.2em}
\begin{center}
\Large
$P \;=\; F_1 \;\times\; F_2 \;\times\; \cdots \;\times\; F_8$
\end{center}
\vspace{1.0em}
\begin{center}
\color{warmgray}\normalsize
Shockley (1957) \textperiodcentered\
\textit{On the Statistics of Individual Variations of Productivity in Research Laboratories}\\[0.6em]
\highlight{Any factor at zero kills output entirely.}
\end{center}
\end{frame}

%% ----------
\begin{frame}{Shockley's eight factors}
\vspace{0.4em}
\begin{columns}[T]
\column{0.48\textwidth}
\begin{enumerate}\small\setlength\itemsep{0.35em}
  \item Ability to think of a good problem
  \item Ability to work on it
  \item Ability to recognize a worthwhile result
  \item Ability to decide when to stop and write up
\end{enumerate}
\column{0.48\textwidth}
\begin{enumerate}\small\setlength\itemsep{0.35em}\setcounter{enumi}{4}
  \item Ability to write adequately
  \item Ability to profit constructively from criticism
  \item Determination to submit the paper to a journal
  \item Persistence in making changes after journal action
\end{enumerate}
\end{columns}
\vspace{0.5em}
\begin{center}\color{warmgray}\small
...
%% ----------
\begin{frame}{APE: 1,000 AI-written papers vs 43 human papers}
\begin{center}
\includegraphics[width=0.80\textwidth,height=0.68\textheight,keepaspectratio]{figures/elo-histogram.png}
\end{center}
\vspace{-0.2em}
\begin{center}\small\color{warmgray}\itshape
Social Catalyst Lab (Stanford). AI papers cluster around human median. \highlight{Zero-shot is not the ceiling.}
\end{center}
\end{frame}
...
%% ----------
\begin{frame}{\texttt{/referee2} --- replicate in a second language}
\vspace{1.0em}
\begin{center}
\large\texttt{/referee2}
\end{center}
\vspace{0.6em}
\begin{center}\normalsize\color{warmgray}
Spawns a skeptical reviewer.\\[0.4em]
Runs your estimation pipeline in R if you wrote it in Stata.\\[0.4em]
Flags every discrepancy.\\[0.6em]
\highlight{If the numbers don't match, something is wrong.}
\end{center}
\end{frame}

%% ----------
\begin{frame}{\texttt{/beautiful\_deck} --- turn results into a presentation}
\vspace{1.0em}
\begin{center}
\large\texttt{/beautiful\_deck}
\end{center}
\vspace{0.6em}
\begin{center}\normalsize\color{warmgray}
Generates a full Beamer deck from your results folder.\\[0.4em]
Applies assertion titles, one idea per slide, clean figures.\\[0.6em]
\highlight{Forces the agent to present its own findings to an audience.}\\[0.4em]
\small If it can't explain the result on a slide, it didn't understand it.
\end{center}
\end{frame}

%% ----------
\begin{frame}{GTD harness --- track the pipeline}
\vspace{1.0em}
\begin{center}
\large GTD (Getting Things Done) harness
\end{center}
\vspace{0.6em}
\begin{center}\normalsize\color{warmgray}
A workflow file that tracks every step:\\[0.4em]
what ran, what verified, what's pending, what failed.\\[0.6em]
\highlight{Multi-step pipelines need a memory.}\\[0.4em]
\small The agent touches 50 files over 4 hours.\\
Without a log, you can't audit what it did.
\end{center}
\end{frame}

%% ====================================================================
%% CONCLUSION — VERIFICATION AND DESKILLING
%% ====================================================================

\begin{frame}{The traditional path from time to output}
\vspace{0.8em}
\begin{center}
\begin{tikzpicture}[
  box/.style={rectangle, rounded corners=4pt, draw=warmgray!60,
              fill=lightbg, minimum width=2.0cm, minimum height=0.75cm,
              align=center, font=\small},
  arr/.style={-{Stealth[length=6pt]}, thick, color=slate},
]
\node[box] (T)  at (0,0)   {Time};
\node[box] (A)  at (3,0)   {Attention};
\node[box] (S)  at (6,0)   {Skill};
\node[box] (O)  at (9,0)   {Output};
\draw[arr] (T.east) -- (A.west);
\draw[arr] (A.east) -- (S.west);
\draw[arr] (S.east) -- (O.west);
\draw[arr, color=accent, dashed, very thick]
  (T.south) .. controls (4.5,-1.4) .. (O.south)
  node[midway, below, font=\small, color=accent] {AI route};
\end{tikzpicture}
\end{center}
\vspace{0.5em}
\begin{center}\color{warmgray}\small
...
%% ----------
\begin{frame}{Production costs have fallen}
\vspace{1.0em}
\begin{center}
\large
\highlight{AER is already seeing a 20--50\% inflow of AI-written submissions.}\\[0.8em]
\color{warmgray}\normalsize
12 editors. Each handling 200 papers. Right now.
\end{center}
\end{frame}
...
%% ----------
\begin{frame}{Submissions are rising --- verification is not}
\vspace{0.8em}
\begin{itemize}\setlength\itemsep{0.6em}
  \item Agents reliably produce output (1): code ran, estimates exist
  \item They do not guarantee correct output (2): right estimator, right design
  \item They do not guarantee interpretable output (3): can you reconstruct every step?
\end{itemize}
\vspace{0.5em}
\emphbox{Production was hard; verification was implicit. \\Now production is cheap; verification is hard.}
\end{frame}

%% ----------
\begin{frame}{You will need to build a verification workflow}
\vspace{0.8em}
\begin{center}
\large
\highlight{Verification is the new bottleneck.}
\end{center}
\vspace{0.6em}
\begin{itemize}\setlength\itemsep{0.5em}
  \item \texttt{/referee2} --- replicate results in a second language or framework
  \item \texttt{/blindspot} --- surface what the analysis missed
  \item \texttt{/beautiful\_deck} --- force the agent to present its own findings
\end{itemize}
\vspace{0.4em}
\begin{center}\color{warmgray}\small
