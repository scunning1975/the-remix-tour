...
twoway (bar county_ate county_index, fcolor(gs12) lcolor(gs9) lwidth(vthin) barwidth(0.7)) ///
    ...
    text(`=`overall_ate'+0.15' `=`total_counties'*0.85' "Overall ATE = `overall_ate_rounded'", place(w) color(gs5) size(small)) ///
    text(`=`county_level_ate'+0.15' `=`total_counties'*0.15' "County Average ATE = `county_level_ate_rounded'", place(e) color(gs7) size(small)) ///
    graphregion(fcolor(white) lcolor(white)) ///
    plotregion(fcolor(white) lcolor(white)) ///
    legend(off) ///
    yscale(range(0 2.4)) ylabel(0(0.5)2.0) ///
